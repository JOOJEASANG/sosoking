...
function fallback(c, judgeType) {
  ...
  return {
    ...
    evidenceList: [
      ...
      `증 제5호: 소소국과수 생활증거 감정요청서`,
      `증 제6호: 피고 측 예상 반박 문장`,
      `증 제7호: 담당 수사관 수첩 기재사항`
    ],
    forensicReport: `문서명: 소소국과수 생활증거 감정서\n감정기관: 국립소소과학수사연구소 생활증거분석실\n감정대상: ${title}과 관련된 사건 대상의 위치, 접촉 가능성, 원고의 기대가 사라진 흔적.\n감정 결과, 제출된 진술만으로 실제 책임을 단정할 수는 없으나 사건 대상이 원고의 관심 영역에서 벗어난 정황은 확인된다. 본 감정서는 실제 국립과학수사기관의 문서가 아니라 소소킹 세계관의 오락용 감정기록이다.`,
    investigation: `문서명: ${analyst} 수사보고서\n수사관은 ${title}의 발생 장면을 사건 경위에 따라 시간순으로 재구성하였다. 수첩에는 사건 대상의 위치, 원고의 시선 또는 주의 상태, 피고 측 접근 가능성이 각각 분리 기재되었다. 현장검증은 사건 대상이 원래 있어야 했던 지점과 결과 발생 후 달라진 지점을 비교하는 방식으로 진행되었다. 탐문은 사건의 크기를 묻는 방식이 아니라 원고가 왜 멈칫했는지를 확인하는 방식으로 정리되었다. 소소국과수 생활증거분석실에는 사건 대상의 흔적과 장면의 부자연스러움에 관한 감정이 의뢰되었다.`,
    ...
    courtOpinion: `문서명: 판결 이유\n${judgeType} 재판부는 접수조서, 수사보고서, 소소국과수 감정서, 준비서면 및 답변서를 종합한다. 본 사건은 현실의 중대한 법률분쟁이 아니나, 원고가 기대하던 흐름이 특정 행위로 끊어진 사실은 인정된다. 재판부는 사건 대상의 사소함과 기록 형식의 장엄함 사이에서 책임의 범위를 제한적으로 정한다.`,
...
7. 소소국과수 감정대상: 현장에서 감정할 수 있는 말도 안 되게 작은 물질, 각도, 냄새, 위치, 시간, 압흔, 빈 공간은 무엇인가.

소소국과수 감정 원칙:
- 실제 국과수라고 쓰지 말고 반드시 '소소국과수', '국립소소과학수사연구소', '소소국과수 생활증거분석실' 같은 가상 기관명으로 쓴다.
- forensicReport는 반드시 별도 감정서로 작성한다.
- 감정 항목은 사건마다 달라야 한다. 모든 사건에 지문, DNA, CCTV를 반복하지 마라.
- 빵 사건이면 빵 부스러기 분포, 침 자국 추정, 포장지 접힘, 리트리버 접근각, 벤치 위 잔여물을 분석한다.
- 리모컨 사건이면 버튼 압흔, 배터리 방향, 소파 쿠션 함몰, 마지막 채널 기록, 손때의 방향성을 분석한다.
- 문 안 닫음 사건이면 문틈 각도, 복도 바람 유입량, 문고리 접촉 흔적, 닫힘 기대 훼손 정도, 문짝의 최종 정지 위치를 분석한다.
- 탕비실 사건이면 카누 봉지 절취선, 컵 주변 분말, 쓰레기통과의 거리, 마지막 스틱 소실 시각, 뜯어진 봉지의 방치 각도를 분석한다.
- 이어폰 사건이면 침대 밑 음영, 먼지 밀림 자국, 케이블 장력, 마지막 청취 위치, 한쪽만 사라진 비대칭성을 분석한다.
- 냉장고 사건이면 빈 공간의 윤곽, 포장지 위치, 냉기 흐름, 디저트 기대 잔존 여부, 선반 위 압흔을 분석한다.
- 감정 결과는 지나치게 진지해야 하며, 사건의 사소함을 직접 비웃지 않는다.
...
- 수사보고서, 소소국과수 감정서, 준비서면, 답변서, 판결 이유의 첫 문장을 서로 비슷하게 시작하지 마라.
...
- 문서명도 사건에 맞게 조금씩 바꿔라. 예: '빵 섭취 경위 수사보고서', '리모컨 점유상태 확인조서', '마지막 푸딩 소실 관련 준비서면', '카누 봉지 절취선 감정서'. 단, 너무 장난스럽게 만들지 마라.
- 수사보고서는 사건에 맞는 도구를 선택한다. CCTV, 수첩, 현장검증, 냄새/자리/시간/물건 위치 확인, 동선 재구성, 주변인 진술, 사진 감정, 대화기록 확인, 냉장고 상태 확인, 테이블 배치 확인 등.
- forensicReport는 '소소국과수 생활증거 감정서' 또는 사건 맞춤 감정서로 쓴다. 감정기관, 감정대상, 감정방법, 감정결과, 감정의견을 포함한다.
...
- forensicReport는 가장 과장된 과학수사 문서다. 그러나 가상기관임을 자연스럽게 드러내라.
- evidenceList의 각 항목은 '증 제N호:'로 시작하고, 사건 대상과 직접 관련되어야 한다. 반드시 소소국과수 감정서 1개를 포함한다.
...
JSON만 출력한다. 다음 필드를 모두 포함한다.
{
  ...
  "evidenceList": ["증 제1호: ...", "증 제2호: ...", "증 제3호: ...", "증 제4호: ...", "증 제5호: 소소국과수 ... 감정서", "증 제6호: ...", "증 제7호: ..."],
  "forensicReport": "문서명: ... 소소국과수 생활증거 감정서 로 시작. 감정기관, 감정대상, 감정방법, 감정결과, 감정의견 포함. 사건별 미세증거 4개 이상. 9문장 이상",
  ...
  "courtOpinion": "문서명: 판결 이유 로 시작. 여기서만 판사 성향 반영. 소소국과수 감정서와 양측 서면을 종합. 11문장 이상",
  ...
  "closingComment": "사건 고유명사를 포함한 장엄한 한 줄"
    ...
    data = {
      ...
      forensicReport: cleanLong(parsed.forensicReport, 3200) || data.forensicReport,
