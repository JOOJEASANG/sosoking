...
const ACTOR_PACKS = [
  {
    ...
    defenseAlias: '꼬리변호인단',
    witnessA: scene => scene.place === '공원' ? '벤치 밑 비둘기 1호' : '현장 주변의 무심한 시선',
    witnessB: () => '목줄을 붙잡고 흔들리던 보호자 손목',
    witnessC: scene => scene.item.includes('빵') ? '입가에 묻은 수상한 빵가루' : `${scene.item} 주변에 남은 수상한 흔적`,
    motive: scene => `${scene.item}의 냄새가 피고의 코를 긴급 호출했다는 주장`,
    excuse: scene => `피고견 측은 “${scene.item}이 먼저 냄새로 저를 불렀습니다. 저는 소환장에 응했을 뿐입니다.”라고 항변하였다.`,
    cross: scene => `검사: “피고는 ${scene.item}을 보았습니까?”\n피고견: “멍.”\n검사: “먹거나 건드렸습니까?”\n피고견: “멍…”\n재판장: “방금 두 번째 멍은 유죄 쪽에 가깝습니다.”`,
    penalty: scene => [`피고견은 원고 앞에서 미안한 눈망울 5초를 집행한다.`, `보호자는 ${scene.item} 또는 동급의 대체 만족물 1개를 원고에게 평화적으로 지급한다.`, `향후 ${scene.item} 발견 시 피고견의 코는 보호자 승인 전까지 접근금지된다.`]
  },
  {
    keys: ['고양이', '냥이', '길고양이'],
    actor: '고양이 피고',
    ...
    witnessA: () => '현장을 애써 외면한 창틀 먼지',
    witnessB: () => '갑자기 그루밍을 시작한 꼬리',
    witnessC: () => '현장 모서리에 남은 발자국 의혹',
    motive: scene => `${scene.item}이 자기 영역 안으로 들어왔다는 주장`,
    excuse: scene => `피고묘 측은 “${scene.item}은 제가 가져간 것이 아니라 우주가 잠시 제 앞에 내려놓은 것입니다.”라고 주장하였다.`,
    cross: scene => `검사: “피고는 ${scene.item}에 접근했습니까?”\n피고묘: “야옹.”\n검사: “그 후 왜 그루밍을 했습니까?”\n재판장: “증거 인멸을 혀로 한 정황이 있습니다.”`,
    penalty: scene => [`피고묘는 원고에게 골골송 5초를 제공한다.`, `집사는 ${scene.item} 상당의 대체 만족물을 원고에게 제공한다.`, `피고묘는 향후 ${scene.item} 앞에서 모른 척하기 전에 최소 1회 눈치를 본다.`]
  },
  {
    ...
    motive: scene => `${scene.item} 문제가 이렇게까지 커질 줄 몰랐다는 주장`,
    excuse: scene => `피고 측은 “${scene.item} 문제는 생활 중 흔히 발생하는 작은 바람소리라고 생각했습니다.”라고 항변하였다.`,
    cross: scene => `검사: “피고는 원고가 ${scene.item} 때문에 굳어가는 것을 보았습니까?”\n피고: “그 정도는 아닌 줄 알았습니다.”\n재판장: “그 말이 바로 이 법정의 연료입니다.”`,
    penalty: scene => [`피고는 ${scene.item} 관련 행동 전 원고 표정을 1회 확인한다.`, `피고는 ‘그럴 수도 있지’ 발언권을 하루 1회로 제한한다.`, `피고는 원고에게 작은 간식 또는 음료로 감정 항소비용을 납부한다.`]
  }
];
...
function detectObject(text) {
  ...
  const tokens = text
    .replace(/[.,!?。！？\n]/g, ' ')
    .split(/\s+/)
    .map(t => t.replace(/(을|를|이|가|은|는|에|에서|에게|한테|으로|로|도|만|까지|부터)$/g, ''))
    .filter(t => t.length >= 2 && !['제가','내가','나는','저는','하고','했는데','있었는데','한눈판사이','사건','내용','접수된','다음과','같다'].includes(t));
  return tokens[0] || '그 일';
}
...
function hasInput(text, terms) {
  return terms.some(t => text.includes(t));
}
function forbiddenTermsForInput(inputText) {
  return CONTAMINATION_RULES.flatMap(rule => hasInput(inputText, rule.mustHaveAny) ? [] : rule.block);
}
function hasForbiddenContamination(outputText, scene) {
  const forbidden = forbiddenTermsForInput(scene.inputText || '');
  return forbidden.some(term => outputText.includes(term));
}
function scenarioFor(c) {
  ...
  const scene = {
    ...sceneBase,
    title: funnyTitleSeed(item, place, actor, seed),
    prosecutorAlias: pack.prosecutorAlias,
    defenseAlias: pack.defenseAlias,
    witnessA: pack.witnessA(sceneBase),
    witnessB: pack.witnessB(sceneBase),
    witnessC: pack.witnessC(sceneBase),
    fakeExhibit: `${item} 주변에 남은 억울함의 테두리`,
    motive: pack.motive(sceneBase),
    excuse: pack.excuse(sceneBase),
    cross: pack.cross(sceneBase),
    penalties: pack.penalty(sceneBase)
  };
...
function fallbackFor(c, judgeType, people) {
  ...
  const absurdDetails = [
    `${s.place}에서 ${s.item}이 원고의 하루 계획을 살짝 흔듦`,
    `${s.actor}이 사건 반경 안으로 자연스럽게 진입함`,
    `${s.fakeExhibit}이 증거 제1호로 제출됨`,
    `${s.witnessA}이 말없이 현장 분위기를 무겁게 만듦`,
    `${s.witnessB}이 사건 직후 이상하게 존재감이 커짐`,
    `원고의 방심 시간이 피고 측 논리의 틈으로 악용됨`,
    `${s.item} 관련 억울함이 공기 중에 둥둥 떠다님`,
    `원고의 빈손 또는 빈마음이 법정에서 가장 큰 소리로 증언함`,
  ...
  const defendantExcuses = [
    s.excuse,
    `${s.defenseAlias}은 원고가 방심한 짧은 시간이 사실상 사건의 열린 문처럼 보였다고 주장하였다.`,
    `피고 측은 ${s.item}이 너무 당당히 존재해 오해를 불렀다고 항변하였다.`,
    `피고 측은 고의가 아니라 상황, 분위기, 순간 판단의 삼자합작이라고 주장하였다.`,
    `다만 피고 측도 사건 직후 원고 표정이 로딩 화면처럼 굳은 사실은 부인하지 못하였다.`
  ];
  const penaltyIdeas = [
    ...s.penalties,
    `원고는 향후 ${s.place}에서 ${s.item} 관련 경계태세를 1단계 올릴 수 있다.`,
    `재판부는 본 사건을 '${s.item} 관련 생활참사'로 마음속 기록철에 편철한다.`,
  ...
  return {
    ...
    expandedCase: [
      '🎬 사건 브리핑',
      openingLine,
      `원고는 ${s.place}에서 ${s.item}과 관련된 평범한 시간을 보내고 있었다. 그러나 잠깐의 방심 또는 예기치 못한 상황 속에서 ${s.actor}이 사건의 중앙무대로 들어왔다.`,
      `검찰은 이를 단순한 해프닝이 아니라 “원고의 일상 계획이 예고 없이 방향을 잃은 사건”으로 규정하였다.`,
      `방청석에서는 ${s.witnessA}과 ${s.witnessB}이 증인으로 거론되자 술렁임이 발생하였다.`,
      `재판장은 “오늘의 쟁점은 ${s.item}이 왜 원고의 의도와 다른 길로 갔는가”라고 말하며 마음속 법봉을 내려쳤다.`
    ].join('\n'),
    caseTimeline: [
      '🎞️ 현장 재연',
      `00:00 원고가 ${s.place}에서 ${s.item} 문제와 관련해 평범한 하루를 보내고 있었다.`,
      `00:02 사건의 틈이 열린다. 훗날 검찰은 이 순간을 “일상질서가 삐끗한 2초”라고 부르게 된다.`,
      `00:03 ${s.actor}이 현장 반경 안으로 들어온다. 등장 자체는 조용했으나, 사건성은 이미 재판장 책상 위에 올라와 있었다.`,
      `00:04 ${s.item} 관련 상황이 원고의 예상과 다른 방향으로 굴러간다. 방청석 상상도에는 이 장면이 슬로모션으로 재생된다.`,
      `00:07 원고가 상황을 확인한다. 손에는 계획만 있고 마음에는 물음표가 남는다.`,
      `00:12 ${s.witnessB}이 현장감을 더하고, ${s.fakeExhibit}은 증거로 굳어지고 있었다.`
    ].join('\n'),
    forensicReport: [
      '🔍 증거 제출 및 감정 결과',
      evidenceBits[0],
      ...
      evidenceBits[2],
      evidenceBits[3],
      `소소국과수 감정 결과, ${s.item}은 단순 요소가 아니라 원고의 당일 감정 스위치에 직접 연결된 핵심 물체 또는 상황으로 판명되었다.`,
      `특히 ${s.actor}의 태연한 태도는 사건 후 원고의 억울함을 전자레인지 2분 30초 수준으로 데운 것으로 보인다.`,
      `감정관 ${people.analystName}: “이건 크기는 작아도 마음속 파장은 중형급입니다.”`
    ].join('\n'),
    plaintiffArg: [
      '🧑‍⚖️ 원고 측 주장',
      `${people.prosecutorName}: “원고는 ${s.item} 문제를 이런 방향으로 넘기겠다고 동의한 적이 없습니다.”`,
      `검찰은 ${s.actor}이 ${s.item}에 관여한 과정이 지나치게 자연스러웠다는 점을 오히려 수상하다고 보았다.`,
      `원고 측은 “잠깐 방심한 것은 맞지만, 그것이 내 하루 계획을 셀프 반납하겠다는 뜻은 아니었다”고 진술하였다.`,
    ...
    defendantArg: [
      '🎭 피고 측 변론',
      `${people.defenderName}: “본 사건은 악의가 아니라 상황의 오해와 순간 판단이 만든 우발적 소동입니다.”`,
      s.excuse,
      `${s.defenseAlias}: “원고의 방심 또는 현장 분위기 때문에 ${s.item}이 무방비 상태처럼 보였습니다.”`,
      s.cross,
      `피고 측은 최후진술에서 “그렇게까지 커질 줄은 몰랐다”는 취지의 태도를 보였다.`
    ].join('\n'),
    courtOpinion: [
      '⚖️ 재판부의 과몰입 판단',
      `${judgeType} 재판부는 세 가지 쟁점을 본다.`,
      `첫째, ${s.item}은 언제 원고의 기대에서 이탈했는가. 둘째, ${s.actor}의 관여는 우발인가, 과감한 현장 진입인가. 셋째, 이 사소한 일이 왜 원고의 하루를 살짝 구겨놓았는가.`,
      `재판부는 피고 측의 ${s.motive}을 일부 이해한다. 그러나 이해한다고 해서 원고의 억울함이 자동 환불되는 것은 아니다.`,
      `재판장은 “사소함은 정상참작 사유이나, 원고의 어이없음을 없던 일로 만드는 만능열쇠는 아니다”라고 판시하였다.`,
    ...
    sentence: [
      ...
      `5. ${penaltyIdeas[4]}`,
      `6. ${penaltyIdeas[5]}`,
      `재판부는 위 명령을 선고하며, ${s.place} 내 모든 ${s.item} 관련 당사자에게 경계심을 1단계 상향할 것을 권고한다.`
    ].join('\n'),
...
function allResultText(data) {
  return [data.refinedCaseTitle, data.absurdityTitle, data.expandedCase, data.caseTimeline, data.forensicReport, data.plaintiffArg, data.defendantArg, data.courtOpinion, data.sentence, data.closingComment, ...(data.absurdDetails || []), ...(data.evidenceBits || []), ...(data.defendantExcuses || []), ...(data.penaltyIdeas || [])].join(' ');
}
function tooGeneric(data, scene) {
  ...
  return boringHits >= 2 || concreteHits < 4 || dialogueHits < 10 || joined.length < 1300 || hasForbiddenContamination(joined, scene);
}
function strictContextNotice(scene) {
  const forbidden = forbiddenTermsForInput(scene.inputText || '');
...
exports.generateTrial = onCall({ region: REGION, secrets: [geminiKey], timeoutSeconds: 300, memory: '512MiB' }, async request => {
  ...
  try {
    const model = new GoogleGenerativeAI(geminiKey.value().trim()).getGenerativeModel({
      model: modelName,
      generationConfig: { temperature: 1.25, topP: 0.98, topK: 64, maxOutputTokens: 8000, responseMimeType: 'application/json' }
    });
