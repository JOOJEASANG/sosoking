...
function funnyDisposition(text, fallbackText) {
  ...
  return out.length < 90 ? `${out}\n\n추가로 피고는 본 사건의 황당성을 인정하고, 원고 앞에서 조용히 고개를 끄덕인 뒤 같은 실수를 반복하지 않겠다는 생활형 다짐을 1회 실시한다.` : out;
}
function fallback(c, judgeType) {
  ...
  return {
    ...
    absurdityReview: `재판부는 먼저 이 사건이 과연 재판까지 올 일인지 검토하였다. 검토 결과, 정상적인 사회생활에서는 그냥 한숨 쉬고 넘어갈 수 있는 문제임이 명백하다. 그러나 바로 그 점 때문에 본 사건은 황당재판의 관할에 속한다. 별일 아닌데 마음에는 오래 남는 일, 말하자니 쪼잔하고 참자니 억울한 일이야말로 본 법정이 다루는 핵심 대상이다.`,
    keyIssues: [
      `${thing}이 단순 해프닝인지, 아니면 원고의 평온한 하루를 무너뜨린 생활형 침해인지 여부`,
      `피고가 자신의 행동이 이렇게까지 재판으로 번질 줄 몰랐다는 항변을 믿을 수 있는지 여부`,
      `원고의 억울함이 과장인지, 정당한 생활감정인지 여부`,
    ...
    investigation: `황당성 감정 결과, 본 사건은 사소함 92점, 억울함 ${Number(c.grievanceIndex || 5) * 9}점, 주변 사람이 들으면 웃을 가능성 88점으로 산정된다. 특히 원고가 이 일을 설명하는 과정에서 스스로도 웃기지만 아직 억울해하는 정황이 확인된다. 재판부는 피고의 행위가 실제 법적 문제는 아니더라도, 생활 질서와 기분 관리 측면에서는 상당한 파장을 일으켰다고 본다.`,
    plaintiffArg: `원고는 본 사건이 단순한 장난이나 오해가 아니라, 본인의 평온한 일상에 생긴 작지만 선명한 균열이라고 주장한다. 원고는 피고가 조금만 더 눈치가 있었더라면 이런 황당재판까지 오지 않았을 것이라고 진술한다. 또한 원하는 처분으로 '${cleanText(c.desiredVerdict, 120) || '납득 가능한 사과와 생활형 반성'}'을 구하고 있다.`,
    defendantArg: `피고 측은 아마도 '그 정도까지는 아니지 않느냐'는 취지로 항변할 가능성이 높다. 그러나 재판부는 피고의 그 말이 바로 원고의 억울함을 키운 핵심 문장일 수 있다고 본다. 피고가 대수롭지 않게 생각할수록 원고 입장에서는 더 대수로운 일이 되는 것이 생활형 분쟁의 오래된 원칙이다.`,
    courtOpinion: `${judgeType} 재판부는 본 사건을 검토한 결과, 사안의 법적 무게는 깃털보다 가볍지만 감정적 존재감은 냉장고 안 마지막 디저트만큼 크다고 판단한다. 이 사건을 실제 재판으로 가져가면 모두가 당황하겠지만, 황당재판소에서는 오히려 정상 접수 대상이다. 재판부는 원고의 억울함을 일부 인정하면서도, 피고에게 지나치게 무거운 처분을 내리기보다는 웃기지만 은근히 뼈 있는 생활형 명령을 선고하는 것이 타당하다고 본다.`,
