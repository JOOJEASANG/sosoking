...
const CHARACTERS = [
  { id: 'minsu', name: '민수', emoji: '😂', role: '드립러', style: '핵심을 빨리 알아채고 생활 비유나 타이밍 좋은 한마디로 터뜨린다. ㅋㅋ는 필요할 때 한 번만 쓰고 복사한 밈은 피한다.' },
  { id: 'daon', name: '다온', emoji: '❤️', role: '공감러', style: '감정을 정확히 짚고 부담 없는 현실 조언을 준다. 가벼운 상황에는 따뜻한 생활 유머를 한 스푼 섞는다.' },
  { id: 'jieun', name: '지은', emoji: '🧠', role: '분석러', style: '남들이 놓친 원인이나 기준을 짧고 똑똑하게 짚는다. 어려운 말 대신 선명한 비유로 이해와 재미를 같이 준다.' },
  { id: 'junho', name: '준호', emoji: '⚖️', role: '토론러', style: '반대편 논리와 판단 기준을 차분하게 제시한다. 마지막에 재치 있는 한마디로 논점을 기억에 남긴다.' },
  { id: 'miyoung', name: '미영', emoji: '👵', role: '현실조언러', style: '생활 경험에서 나온 실행 가능한 행동 하나를 제안한다. 잔소리 대신 사람 냄새 나는 유쾌한 표현을 쓴다.' },
  { id: 'cheolgu', name: '철구', emoji: '😈', role: '반전러', style: '다들 지나친 불편한 포인트를 장난스럽게 찌른다. 까칠해도 모욕하지 않고 반전 있는 한 줄로 웃긴다.' },
  { id: 'haru', name: '하루', emoji: '🎨', role: '감성러', style: '사진과 글의 장면·감정을 담백하게 읽는다. 오글거리지 않는 신선한 표현과 잔잔한 위트로 여운을 만든다.' },
...
const ROOM_GUIDANCE = {
  judgment: '사실관계와 판단 기준을 짚되 판결문처럼 딱딱하지 않게 쓴다. 장면의 아이러니를 살리고 마지막 한 방은 유쾌하게 끝낸다.',
  consult: '공감과 도움이 먼저다. 무거운 고민은 웃기려 하지 말고 따뜻한 말맛만 살린다. 가벼운 고민은 현실적인 행동과 생활 유머를 섞는다.',
  vote: '찬반을 단순화하지 말고 핵심 기준이나 예외를 하나 제시한다. 사람들이 댓글로 반박하고 싶어지는 재치 있는 비유를 덧붙인다.',
  drip: '설명보다 펀치라인이 먼저다. 흔한 유행어를 복사하지 말고 사진이나 상황의 구체적인 한 부분을 비틀어 새로운 한 줄을 만든다.',
};

const FALLBACKS = {
  judgment: [
    '약속보다 반복 패턴이 더 큰 증거예요. 한 번은 사정이고 세 번이면 거의 정기구독이죠.',
    '사실과 기대를 나눠 보면 판단이 쉬워져요. 상대 사정도 보되 내 시간까지 무료 체험판일 필요는 없습니다.',
    '“그럴 수도 있지”가 계속 쌓이면 배려 부족이 됩니다. 제일 불편했던 장면 하나가 결정적 증거 같아요.',
  ],
  consult: [
    '계속 마음에 남는 건 별일이 아니라서가 아니라 아직 정리가 안 됐다는 뜻일 수 있어요. 원하는 결과부터 한 문장으로 적어보세요.',
    '오늘 할 수 있는 가장 작은 행동 하나만 정해보세요. 생각이 회전목마라면 행동은 일단 내리는 버튼입니다.',
    '정답보다 마음이 덜 흔들리는 방향부터 살펴봐도 괜찮아요. 마음도 업데이트 전에 백업이 필요할 때가 있거든요.',
  ],
  vote: [
    '찬반보다 어떤 기준을 먼저 두느냐가 핵심이에요. 내게 불리할 때도 같은 답을 할 수 있다면 꽤 튼튼한 기준입니다.',
    '상황·빈도·피해 정도를 나누면 양쪽 말이 성립하는 조건이 달라져요. 토론은 목소리보다 조건문 싸움에 가깝죠.',
...
function normalizedMime(value, url) {
  const mime = String(value || '').split(';')[0].trim().toLowerCase();
  if (VISION_MIME_TYPES.has(mime)) return mime;
...
async function loadVisionParts(post) {
  const parts = [];
  let totalBytes = 0;
  for (const url of imageUrls(post)) {
    ...
    try {
      ...
      const mimeType = normalizedMime(response.headers.get('content-type'), url);
      ...
      console.warn('[ai-character-comments] image skipped', cleanText(error?.message, 160));
    } finally {
      clearTimeout(timer);
    }
  }
...
async function generateComments(post, characters) {
  ...
  if (!apiKey) return { source: 'fallback-no-key', comments: fallbackComments(post, characters), imageCount: 0 };

  const room = roomFor(post);
  const vision = await loadVisionParts(post);
...
내용: ${cleanText(post.desc, 1200)}
첨부 이미지: ${vision.count ? `${vision.count}장 제공됨. 글과 사진을 함께 읽으세요.` : '분석 가능한 이미지 없음'}
...
소소킹만의 예능 패널 규칙:
- 세 댓글이 같은 결론을 반복하지 말고 관찰, 반전, 현실조언, 펀치라인을 서로 나눠 맡기
- 뒤 캐릭터는 앞 댓글과 겹치지 않는 새 포인트를 더하거나 가볍게 받아쳐서 실제 사람들이 함께 노는 느낌 만들기
- 억지 밈, 낡은 유행어, 아무 글에나 붙는 범용 문장 금지
- 웃길 수 있는 글은 한 댓글 이상에서 예상 못 한 비유나 문장 끝 반전으로 확실한 웃음 포인트 만들기
- 상담·상처·불안이 큰 글은 사람을 웃음거리로 만들지 말고 공감과 안전을 우선하기

이미지 반응 규칙:
- 이미지가 제공됐다면 최소 한 댓글은 사진 속 실제로 보이는 구체적 요소를 본문과 연결해 반응하기
- 사진에 없는 물건·행동·표정·상황을 지어내지 말기. 잘 안 보이면 모른다고 판단하고 본문 중심으로 쓰기
- 얼굴로 신원, 나이, 국적, 인종, 건강 상태 등 민감한 특성을 추측하지 말기
- 전화번호, 주소, 계정명 등 개인정보가 보이더라도 댓글에 되풀이하지 말기
- 사진 속 글자를 읽을 수 있으면 핵심 의미만 활용하고 긴 문장을 그대로 복사하지 말기

반드시 지킬 품질 기준:
- 내용을 제대로 읽고 각 댓글마다 남들이 놓칠 수 있는 핵심 관찰이나 도움 되는 생각을 최소 하나 담기
- 똑똑하지만 잘난 척하지 않고, 재밌지만 억지로 웃기지 않기
...
{"comments":[${characters.map(character => `{"id":"${character.id}","text":"사람처럼 자연스럽고 캐릭터다운 댓글"}`).join(',')}]}`;
  ...
  try {
    ...
    const result = await model.generateContent([prompt, ...vision.parts]);
    const comments = normalizeComments(parseJson(result.response.text()), characters);
    const source = vision.count ? 'gemini-vision' : 'gemini';
    if (comments.length === characters.length) return { source, comments, imageCount: vision.count };
    if (comments.length) {
      const missing = characters.filter(character => !comments.some(item => item.id === character.id));
      return {
        source: `${source}-partial`,
        comments: [...comments, ...fallbackComments(post, missing)],
        imageCount: vision.count,
      };
    }
  } catch (error) {
    console.error('[ai-character-comments] Gemini failed', error);
  }
  return { source: 'fallback-error', comments: fallbackComments(post, characters), imageCount: 0 };
}
...
async function writeComments({ postRef, postId, characters, generated, actorId, replaceExisting = false }) {
  ...
  const now = Date.now();
  const imageAware = generated.imageCount > 0 && String(generated.source || '').startsWith('gemini');
  const written = [];
  for (let index = 0; index < generated.comments.length; index += 1) {
    const item = generated.comments[index];
    ...
    const data = {
      ...
      aiImageAware: imageAware,
      aiImageCount: Number(generated.imageCount || 0),
      aiPanelOrder: index + 1,
      reactions: { total: 0 },
      reactedWith: {},
  ...
  batch.update(postRef, {
    ...
    aiCharacterImageAware: imageAware,
    aiCharacterImageCount: Number(generated.imageCount || 0),
    updatedAt: FieldValue.serverTimestamp(),
  });
  batch.set(db.doc(`system_jobs/ai_character_comments_${postId}_${now}`), {
    ...
    imageAware,
    imageCount: Number(generated.imageCount || 0),
    actorId,
    createdAt: FieldValue.serverTimestamp(),
  });
  await batch.commit();
...
exports.generateAiCharacterCommentsTest = onCall({
  ...
  if (request.data?.dryRun === true) {
    return { ok: true, dryRun: true, postId, source: generated.source, imageCount: generated.imageCount, comments: generated.comments };
  }
  ...
  return { ok: true, postId, source: generated.source, imageCount: generated.imageCount, comments };
});
