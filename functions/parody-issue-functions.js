...
// parody-issue-functions.js — 매일 오전 8시 역사정치 자료 3건 자동 생성
...
function daysBetween(a, b) {
  const start = new Date(`${a}T00:00:00+09:00`).getTime();
  const end = new Date(`${b}T00:00:00+09:00`).getTime();
...
function pickEventForSlot(date, slot, startDay = null) {
  ...
  const baseDay = startDay ? Number(startDay) : (daysBetween(HISTORY_SEQUENCE_ANCHOR, date) * DAILY_HISTORY_ISSUE_COUNT) + 1;
...
async function runGenerateDailyParodyIssues(options = {}) {
  ...
  if (cacheSnap.exists && cacheSnap.data()?.done && cachedItems.length >= count && !options.force) {
    return { skipped: true, date: today, count: cachedItems.length, items: cachedItems, sequential: true };
  }
  ...
  for (let slot = 0; slot < count; slot += 1) {
    ...
    items.push({ slot: publicSlot, postId, historyDay: event.day, motifYear: event.motifYear, motif: event.motif, title: clean(issue.title || event.parodyTitle, 70) });
  }
  ...
  await cacheRef.set({
    ...
    sequential: true,
    anchor: HISTORY_SEQUENCE_ANCHOR,
    detailed: true,
    generatedAt: FieldValue.serverTimestamp(),
  }, { merge: true });
  return { skipped: false, date: today, count: items.length, items, sequential: true, detailed: true };
}
...
exports.previewHistoryIssue = onCall({ region: REGION, timeoutSeconds: 30 }, async req => {
  ...
  return { ok: true, date, count, event: events[0], events, cached: cacheSnap && cacheSnap.exists ? cacheSnap.data() : null, sequential: true };
});
