...
function buildBattlePrompt() {
  ...
  return `소소킹 왕국의 오늘의 코미디 정치 드라마를 생성하라.
...
exports.generateDailyBattle = onSchedule({
  ...
  let raw = '';
  try {
    raw = await callAI(buildBattlePrompt(), 3500);
...
exports.closeDailyBattle = onSchedule({
  ...
  if (winner) {
    ...
    await db.doc(`charStats/${winner}`).set({
...
exports.adminGenerateBattle = onCall({
  ...
  const raw = await callAI(buildBattlePrompt(), 3500);
  const parsed = safeParseJson(raw);
