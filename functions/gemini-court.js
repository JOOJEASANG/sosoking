...
function geminiSchema(value) {
  ...
  const allowed = new Set(["type", "properties", "required", "items", "enum", "description"]);
  const result = {};
  for (const [key, item] of Object.entries(value)) {
    if (allowed.has(key)) result[key] = geminiSchema(item);
  }
  return result;
}
...
async function createGeminiClient(apiKey) {
  ...
  const { GoogleGenAI } = await sdkPromise;
...
function assertSafeResponse(response) {
  ...
  if (promptBlock || finishReason === "SAFETY") {
    throw new SafetyInputError("가벼운 일상 소재만 접수할 수 있습니다.");
  }
}
...
사소한 원문 사실을 유지하되 의미와 절차만 터무니없이 확대하라.${correctionText}`;
  ...
  const response = await withTimeout(ai.models.generateContent({
    ...
    config: {
      ...
      responseSchema: geminiSchema(CASE_SCHEMA),
