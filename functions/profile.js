...
exports.getGamePlayerProfiles = onCall({ region: REGION, timeoutSeconds: 20, memory: '256MiB' }, async request => {
  ...
  await enforceActionRateLimit(uid, 'game-profile-read', {
    cooldownSeconds: 1,
    dailyLimit: 500
  });

  const memberRef = db.doc(`game_rooms/${roomId}/players/${uid}`);
  ...
  const userRefs = playerUids.map(playerUid => db.doc(`users/${playerUid}`));
