...
exports.getAiKingUsage = onCall({ region: 'asia-northeast3' }, async (request) => {
  ...
  const today = kstToday();
  const [judgeSnap, userSnap, config] = await Promise.all([
    db.doc(`ai_king_usage/${userId}_${today}_judge`).get(),
    db.doc(`users/${userId}`).get(),
    getAiKingConfig(),
  ]);
  return {
    judge: judgeSnap.exists ? (judgeSnap.data().count || 0) : 0,
    extraUses: userSnap.exists ? (userSnap.data()?.extraAiUses || 0) : 0,
    dailyFreeLimit: config.dailyFreeLimit || DAILY_LIMIT,
  };
});
