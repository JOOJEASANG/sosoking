...
function cleanId(value) { return String(value || '').replace(/[\u0000-\u001F\u007F]/g, '').trim().slice(0, 180); }
...
async function writeAdminLog(uid, action, caseId, detail = {}) { await db.collection('admin_logs').add({ uid, action, caseId, detail, createdAt: FieldValue.serverTimestamp() }).catch(() => null); }
...
exports.deleteCourtPost = onCall({ region: REGION, timeoutSeconds: 120, memory: '256MiB' }, async request => {
  ...
  await deleteQuerySnapshot(db.collection(`result_ratings/${caseId}/ratings`), counter);
  ...
  const refs = [db.doc(`result_reactions/${caseId}`), db.doc(`result_ratings/${caseId}`), db.doc(`court_comment_stats/${caseId}`), db.doc(`court_comments/${caseId}`), db.doc(`results/${caseId}`), db.doc(`cases/${caseId}`)];
  const batch = db.batch();
  refs.forEach(ref => batch.delete(ref));
  await batch.commit();
  counter.deleted += refs.length;
  await writeAdminLog(request.auth.uid, 'deleteSosoRecord', caseId, counter);
