...
const { db, REGION, FieldValue, HttpsError, cleanId, cleanNickname, nicknameKey, assertAdmin, writeAdminLog } = require('./admin-utils');
...
async function deleteStoragePrefix(prefix, counter) {
  ...
  } catch (err) { counter.storageDeleteError = err.message || String(err); }
}
...
exports.deleteMyCase = onCall({ region: REGION, timeoutSeconds: 120, memory: '256MiB' }, async request => {
  ...
  const [caseSnap, resultSnap] = await Promise.all([db.doc(`cases/${caseId}`).get(), db.doc(`results/${caseId}`).get().catch(() => null)]);
  ...
  const counter = { deleted: 0, storageDeleted: 0 };
...
exports.deleteUserProfile = onCall({ region: REGION, timeoutSeconds: 120, memory: '256MiB' }, async request => {
  ...
  const oldKey = data.nickname ? nicknameKey(cleanNickname(data.nickname)) : '';
  const counter = { deleted: 0, storageDeleted: 0 };
  const batch = db.batch();
