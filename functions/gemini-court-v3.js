...
function validate(body) {
  ...
  if (privateData(incident)) return { error: "전화번호·이메일·주소·차량번호 등 개인정보를 삭제해주세요." };
...
function auditCourtCase(data) {
  ...
  if (normalize(data.prosecution) === normalize(data.defense)) issues.push("공방 동일");
...
function parseResponse(response) {
  ...
  try {
    ...
    throw new Error(`Gemini JSON 파싱 실패(${reason}): ${error.message}`);
  }
}

async function generate(apiKey, incident, severity, correction = "") {
  const { GoogleGenAI, Type, HarmCategory, HarmBlockThreshold } = await sdk();
...
function diagnostic(error) {
  return {
    status: String(error?.status || error?.code || "GENERATION").slice(0, 40),
...
exports.generateCourtCase = onRequest(
  ...
  async (req, res) => {
    ...
    if (origin && !ORIGINS.has(origin) && !previewOrigin(origin)) return res.status(403).json({ error: "허용되지 않은 요청 출처입니다." });
    if (req.method !== "POST") {
      res.set("Allow", "POST");
    ...
    if (limited(clientIp(req))) return res.status(429).json({ error: "재판부가 과로 중입니다. 잠시 뒤 다시 접수해주세요." });
