const PII_PATTERNS = [
  ...
  },
  {
    ...
    pattern: /(?:이름|성명|실명)\s*[:：]?\s*[가-힣]{2,4}(?=\s|$|[,.!?])/,
    message: '실명은 입력할 수 없습니다. 관계를 나타내는 표현이나 별칭을 사용해 주세요.'
  },
  {
    code: 'person-name-honorific',
    pattern: /(?:김|이|박|최|정|강|조|윤|장|임|한|오|서|신|권|황|안|송|전|홍|유|고|문|양|손|배|백|허|남|심|노|하|곽|성|차|주|우|구|민|진|엄|채|원|천|방|공|현|함|변|염|여|추|도|소|석|선|설|마|길|연|위|표|명|기|반|왕|금|옥|육|인|맹|제|모|탁|국|어|은|편|용)[가-힣]{1,2}\s*(?:씨|님|군|양)(?:은|는|이|가|을|를|에게|께서|와|과|도|의)?(?=\s|$|[,.!?])/,
    message: '실명으로 보이는 표현은 입력할 수 없습니다. 관계를 나타내는 표현이나 별칭을 사용해 주세요.'
  }
];
