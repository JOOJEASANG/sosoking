...
function missionFallback(today) {
  const pool = [
    {
      title: '오늘 먹은 것 중 제일 소소하게 행복했던 메뉴는?',
      desc: '사진이 없어도 좋아요. 오늘 먹은 음식 중 의외로 기분 좋아졌던 메뉴를 공유해보세요.',
      cat: 'malhe',
      type: 'story',
      prompt: '오늘의 소소한 음식 행복 공유하기',
    },
    {
      title: '퇴근 후 1시간이 생긴다면 뭐 할래요?',
      desc: '잠깐의 자유시간을 어떻게 쓰고 싶은지 골라보고 댓글로 이유를 남겨보세요.',
      cat: 'golra',
      type: 'vote',
      prompt: '퇴근 후 1시간 사용법 투표하기',
    },
    {
      title: '오늘 하루를 한 단어로 작명한다면?',
      desc: '오늘 기분, 사건, 실수, 웃긴 순간을 담아 하루 제목을 붙여보세요.',
      cat: 'usgyo',
      type: 'naming',
      prompt: '오늘 하루 제목 짓기',
    },
...
function normalizeMission(raw, today) {
  ...
  const title = cleanText(data.title || fallback.title, 80);
  const desc = cleanText(data.desc || fallback.desc, 500);
  const prompt = cleanText(data.prompt || data.participationPrompt || fallback.prompt || title, 180);
...
async function deactivateExpiredAndOldAiMissions(today) {
  ...
  expiredSnap.docs.forEach(doc => {
    batch.update(doc.ref, { active: false, autoClosedAt: FieldValue.serverTimestamp() });
    count += 1;
  });
  oldAiSnap.docs.forEach(doc => {
    const data = doc.data() || {};
    if (data.aiGeneratedDate !== today) {
...
function buildPrompt(today) {
  return `당신은 한국 놀이형 커뮤니티 "소소킹"의 미션 운영 AI입니다.
오늘 날짜는 ${today}입니다.
사용자가 바로 글을 쓰거나 댓글을 달고 싶어지는 가벼운 오늘의 미션 1개를 만드세요.

반드시 유효한 JSON 객체만 반환하세요. 마크다운 코드블록, 설명, 주석 금지.
{
  "title": "80자 이내의 오늘의 미션 제목",
  "desc": "사용자가 어떻게 참여하면 되는지 2문장 이내로 설명",
  "cat": "golra 또는 usgyo 또는 malhe",
  "type": "balance 또는 vote 또는 naming 또는 acrostic 또는 howto 또는 story",
  "prompt": "글쓰기 화면에 연결할 참여 유도 문장"
}

카테고리 규칙:
- golra: 고르기, 투표, 밸런스게임
- usgyo: 웃긴 작명, 삼행시, 드립
- malhe: 경험담, 고민, 하우투, 이야기

금지:
- 정치, 종교, 혐오, 성적 내용, 실명 비방, 위험한 행동
- 너무 어려운 미션
- 사진 필수 미션
- 현금/상품 요구
...
async function createAiMission({ force = false, actorId = 'system' } = {}) {
  ...
  if (markerSnap.exists && !force) {
    return { created: false, reason: 'already-exists', missionId: markerSnap.data().missionId || null };
  }
  ...
  return { created: true, missionId: missionRef.id, mission: { id: missionRef.id, ...missionData } };
}

const generateAiMissionNow = onCall(
  { region: REGION, timeoutSeconds: 60, secrets: ['ANTHROPIC_API_KEY'] },
  async (request) => {
...
const dailyAiMission = onSchedule(
  {
    schedule: '5 8 * * *',
    timeZone: 'Asia/Seoul',
    region: REGION,
