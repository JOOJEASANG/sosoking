...
export function openShareSheet(post) {
  ...
  const aiShareText = AI_SHARE_TEXTS[post.type]?.(post, url);
  ...
  sheet.innerHTML = `
    ...
    <div class="share-sheet__panel">
      ...
      <div class="share-sheet__grid">
        ...
        ${aiShareText ? `
        <button class="share-btn share-btn--text" id="btn-copy-text">
          <span class="share-btn__icon">📋</span><span class="share-btn__label">결과 텍스트</span>
        </button>` : ''}
      </div>
    </div>`;
  ...
  sheet.querySelector('#btn-copy-text')?.addEventListener('click', async () => {
    await copyToClipboard(aiShareText);
    toast.success('결과 텍스트가 복사됐어요! 카톡에 붙여넣어 보세요 📋');
    sheet.remove();
  });
