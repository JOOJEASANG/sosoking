// civic-plaza-copy.js
// 기존 /feed 경로는 유지하되, 시민광장을 공화국 여론/토론 게시판으로 보이게 정리합니다.
...
function ensureStyle() {
  ...
  style.textContent = `
    ...
    .civic-plaza-hero__top{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap}
    .civic-plaza-hero__title{font-size:22px;font-weight:1000;line-height:1.2}
    .civic-plaza-hero__desc{font-size:13px;line-height:1.55;color:rgba(255,255,255,.76);margin-top:6px}
    ...
    .civic-plaza-hero__write{border:0;border-radius:999px;padding:10px 13px;background:var(--color-primary);color:#fff;font-weight:1000;font-family:inherit;cursor:pointer;box-shadow:0 8px 18px rgba(255,107,74,.22)}
    @media(max-width:420px){.civic-plaza-hero{border-radius:18px;padding:13px}.civic-plaza-hero__title{font-size:19px}.civic-plaza-hero__write{width:100%}}
  `;
...
function updateMeta() {
  ...
  document.title = '소소킹 시민광장 - 공화국 여론 게시판';
  const desc = '소소킹 시민광장은 정당 홍보, 공약 토론, 정치 의견, 헌재 기록이 모이는 공화국 여론 게시판입니다.';
...
function addCivicPlazaHero() {
  ...
  hero.innerHTML = `
    <div class="civic-plaza-hero__top">
      <div>
        <div class="civic-plaza-hero__eyebrow">REPUBLIC OPINION</div>
        <div class="civic-plaza-hero__title">🏛️ 시민광장</div>
        <div class="civic-plaza-hero__desc">시민광장은 공화국의 여론이 모이는 공간입니다. 정당 홍보, 공약 토론, 정치 의견, 헌재 기록을 한곳에서 확인하세요.</div>
      </div>
      <button type="button" id="civic-plaza-write-btn" class="civic-plaza-hero__write">시민발언 작성 +20P</button>
    </div>
    <div class="civic-plaza-hero__chips">
      <span class="civic-plaza-hero__chip">시민발언 +20P</span>
      <span class="civic-plaza-hero__chip">시민토론 +10P</span>
      <span class="civic-plaza-hero__chip">정당 홍보</span>
      <span class="civic-plaza-hero__chip">공약 토론</span>
      <span class="civic-plaza-hero__chip">헌재기록 보관</span>
    </div>`;
  page.insertBefore(hero, page.firstElementChild);
  hero.querySelector('#civic-plaza-write-btn')?.addEventListener('click', () => navigate('/write'));
}
