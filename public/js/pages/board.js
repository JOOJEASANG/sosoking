...
function kingVotes(r) {
  return Number(r.kingCount || 0);
}
function totalComments(r) {
  return Number(r.commentCount || 0);
}
function kingScore(r) {
  return kingVotes(r) * 10 + totalVotes(r) * 2 + totalComments(r);
}
...
export async function renderBoard(container) {
    ...
    <div>
      ...
      <div class="container" style="padding-top:22px;padding-bottom:90px;">
        <div style="margin-bottom:18px;">
          ...
          <div style="font-size:13px;color:var(--cream-dim);line-height:1.7;">모기, 라면, 리모컨 같은 한 줄 소소사건 기록입니다. 가장 웃긴 기록에 <b style="color:var(--gold);">소소킹감</b>을 투표하면 킹 후보로 올라갑니다.</div>
        </div>
  ...
  try {
    ...
    const ranked = [...rows].sort((a, b) => kingScore(b[1]) - kingScore(a[1]));
    const top = ranked[0];
    document.getElementById('today-pick').innerHTML = top ? todayPick(top) : '';
    document.getElementById('king-list').innerHTML = ranked.length ? kingList(ranked.slice(0, 3)) : '';
...
function todayPick([id, r]) {
  ...
  return `<div class="card" onclick="location.hash='#/result/${encodeURIComponent(id)}'" style="padding:20px;margin-bottom:16px;cursor:pointer;border-color:rgba(201,168,76,.75);background:linear-gradient(135deg,rgba(201,168,76,.18),rgba(231,76,60,.12),rgba(255,255,255,.03));">
    <div style="font-size:12px;color:var(--gold);font-weight:900;letter-spacing:.12em;margin-bottom:8px;">👑 오늘의 소소킹</div>
    ...
    <div style="display:flex;justify-content:space-between;gap:8px;font-size:12px;color:var(--cream-dim);"><span>${icon} ${escapeHtml(r.judgeType || 'AI')} 위원</span><span>👑 ${kingVotes(r)} · 🧑‍💼 ${totalVotes(r)} · 💬 ${totalComments(r)}</span></div>
  </div>`;
}

function kingList(rows) {
  return `<div style="margin-bottom:16px;">
    <div style="font-size:13px;color:var(--cream-dim);margin:0 0 8px;">👑 소소킹 후보 TOP 3</div>
    <div style="display:grid;grid-template-columns:1fr;gap:8px;">
      ${rows.map(([id, r], idx) => `<div class="card" onclick="location.hash='#/result/${encodeURIComponent(id)}'" style="padding:13px 14px;cursor:pointer;display:flex;gap:10px;align-items:center;">
        ...
        <div style="flex:1;min-width:0;"><div style="font-weight:900;font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(r.caseTitle || '제목 없음')}</div><div style="font-size:12px;color:var(--cream-dim);margin-top:2px;">👑 ${kingVotes(r)}표 · ${escapeHtml(summaryOf(r, 44))}</div></div>
      </div>`).join('')}
    </div>
  </div>`;
}
...
function boardRow(id, r, idx = 0) {
  ...
  const rank = kingVotes(r) >= 3 ? '소소킹 후보' : '긴급기록';
  return `<div class="card" onclick="location.hash='#/result/${encodeURIComponent(id)}'" style="padding:16px 18px;cursor:pointer;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;font-size:12px;"><span style="color:var(--cream-dim);">${icon} ${escapeHtml(r.judgeType || 'AI')} 위원 · ${rank}</span><span style="color:var(--gold);white-space:nowrap;">👑 ${kingVotes(r)} · 🧑‍💼 ${totalVotes(r)} · 💬 ${totalComments(r)} →</span></div>
