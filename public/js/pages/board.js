...
export async function renderBoard(container) {
  ...
  try {
    ...
    document.getElementById('today-pick').innerHTML = top ? popularPick(top) : '';
    list.innerHTML = `<div style="font-size:13px;color:var(--cream-dim);margin:18px 0 8px;">📜 최근 공개 황당판결 <span style="font-size:11px;opacity:.72;">최근 40건 기준</span></div><div style="display:flex;flex-direction:column;gap:10px;">${rows.map(row => boardRow(...row)).join('')}</div>`;
  } catch (err) {
    console.error(err);
