...
export async function renderBoard(container) {
    ...
    <div>
      ...
      <div class="container" style="padding-top:22px;padding-bottom:90px;">
        <div style="margin-bottom:18px;">
          <div style="font-family:var(--font-serif);font-size:22px;font-weight:900;color:var(--gold);margin-bottom:6px;">공개 긴급 결정 기록</div>
          <div style="font-size:13px;color:var(--cream-dim);line-height:1.7;">다른 사람들이 공개한 한 줄 분쟁 기록입니다. 속보와 위원회 결정을 보고 시민 의견과 댓글을 남길 수 있습니다.</div>
        </div>
  ...
  try {
    ...
    if (snap.empty) {
      list.innerHTML = `<div style="text-align:center;padding:52px 0;color:var(--cream-dim);"><div style="font-size:46px;margin-bottom:12px;">📭</div>아직 공개된 긴급 결정 기록이 없습니다.<br><a href="#/submit" style="color:var(--gold);margin-top:12px;display:inline-block;">첫 분쟁 접수하기</a></div>`;
      return;
    }
    ...
    list.innerHTML = `<div style="font-size:13px;color:var(--cream-dim);margin:18px 0 8px;">📡 최근 공개 긴급기록</div><div style="display:flex;flex-direction:column;gap:10px;">${rows.map(row => boardRow(...row)).join('')}</div>`;
  } catch (err) {
    console.error(err);
...
function todayPick([id, r]) {
  ...
  return `<div class="card" onclick="location.hash='#/result/${encodeURIComponent(id)}'" style="padding:20px;margin-bottom:16px;cursor:pointer;border-color:rgba(201,168,76,.65);background:linear-gradient(135deg,rgba(231,76,60,.12),rgba(201,168,76,.12),rgba(255,255,255,.03));">
    <div style="font-size:12px;color:var(--gold);font-weight:900;letter-spacing:.12em;margin-bottom:8px;">오늘의 긴급 결정</div>
    ...
    <div style="display:flex;justify-content:space-between;gap:8px;font-size:12px;color:var(--cream-dim);"><span>${icon} ${escapeHtml(r.judgeType || 'AI')} 위원</span><span>🧑‍💼 ${totalVotes(r)}표 · 💬 ${totalComments(r)}</span></div>
  </div>`;
}
...
function boardRow(id, r) {
  ...
  return `<div class="card" onclick="location.hash='#/result/${encodeURIComponent(id)}'" style="padding:16px 18px;cursor:pointer;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;font-size:12px;"><span style="color:var(--cream-dim);">${icon} ${escapeHtml(r.judgeType || 'AI')} 위원 · 사소함 ${escapeHtml(r.grievanceIndex || '?')}/10</span><span style="color:var(--gold);white-space:nowrap;">🧑‍💼 ${totalVotes(r)} · 💬 ${totalComments(r)} →</span></div>
    ${isDaily ? `<div style="margin-top:8px;font-size:11px;color:var(--gold);font-weight:800;">오늘의 AI 분쟁</div>` : ''}
  </div>`;
}