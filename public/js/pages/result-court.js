import { renderResult as renderBaseResult } from './result.js?v=20260702-11';
...
function badgesBy(container, lv) {
  ...
  if (text.includes('엄벌주의형')) badges.push(['📋', '엄중 판단']);
...
function ensureResultGameStyle() {
  ...
  style.textContent = `
    ...
    .reward-grade{width:70px;height:70px;border-radius:20px;display:flex;align-items:center;justify-content:center;font-family:var(--font-serif);font-size:32px;font-weight:900;color:#111827;background:linear-gradient(135deg,#ffdf7a,#ff7166);box-shadow:0 10px 26px rgba(231,76,60,.20);}
    ...
    <div id="game-reward-card" class="reward-card">
      <div style="display:flex;gap:14px;align-items:center;margin-bottom:14px;">
        ...
        <div style="flex:1;min-width:0;">
          <div class="court-kicker">BREAKING REWARD</div>
          <div class="court-title" style="font-size:20px;">${label}</div>
          <div class="court-desc">아무것도 아닌 한 줄이 긴급속보와 위원회 결정으로 기록되었습니다.</div>
        </div>
      </div>
...
function addInviteDefense(container) {
  ...
  actions.insertAdjacentHTML('beforebegin', `
    <div id="invite-defense-card" class="invite-defense">
      <div class="invite-defense-title">📡 친구 의견 소집 준비중</div>
      <div class="invite-defense-desc">친구를 제보자 편/상대 편 시민 의견으로 초대하는 기능을 붙일 수 있습니다. 지금은 결과 링크를 복사해 공유할 수 있습니다.</div>
  ...
  if (titleCard && !document.getElementById('court-result-header')) {
    ...
    titleCard.insertAdjacentHTML('afterbegin', `
      <div id="court-result-header" style="display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:10px;">
        ...
        <span class="court-kicker">BREAKING COMMITTEE CLEAR</span>
      </div>
  ...
  if (verdictCard && !document.getElementById('court-verdict-label')) {
    ...
    verdictCard.insertAdjacentHTML('afterbegin', `
      <div id="court-verdict-label" style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:12px;">
        <div><div class="court-kicker">SOSO DISPUTE COMMITTEE</div><div class="court-title" style="font-size:19px;">위원회 결정문</div></div>
