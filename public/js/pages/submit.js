...
const JUDGES = [
  ...
  { id: '과몰입형', icon: '🔥', desc: '한 줄 사건을 국가 의제로 확대' },
  { id: '피곤형', icon: '😴', desc: '귀찮지만 결정문은 또 씀' },
  { id: '논리집착형', icon: '🧮', desc: '하찮음을 수치화해서 처리' },
  { id: '드립형', icon: '🎭', desc: '속보 톤으로 정색 개그' }
];

function compact(v, max = 80) {
...
export async function renderSubmit(container) {
    ...
    <div>
      ...
      <div class="container" style="padding-top:24px;padding-bottom:80px;">
        <div class="card" style="padding:18px;margin-bottom:18px;border-color:rgba(201,168,76,.45);">
          <div style="font-size:11px;color:var(--gold);font-weight:900;letter-spacing:.12em;margin-bottom:6px;">BREAKING SOSO</div>
          <div style="font-family:var(--font-serif);font-size:21px;font-weight:900;line-height:1.45;">소소긴급위원회 긴급접수</div>
          <div style="font-size:13px;color:var(--cream-dim);line-height:1.7;margin-top:6px;">모기, 라면, 리모컨 같은 한 줄 소소사건을 넣으면 속보 → 브리핑 → 쟁점 → 위원회 결정 → 소소 처분으로 처리합니다. 공개 기록에서는 유저가 웃김 점수를 매깁니다.</div>
        </div>
        <form id="submit-form">
          <div class="form-group">
            <label class="form-label">오늘의 소소사건 <span style="color:var(--red)">*</span></label>
            <textarea id="quick-brief" class="form-textarea" style="min-height:88px;" maxlength="${MAX_QUICK}" placeholder="예: 오늘 모기 때문에 잠을 못 잠"></textarea>
            <div class="char-counter"><span id="quick-count">0</span>/${MAX_QUICK}</div>
            <div style="display:flex;flex-wrap:wrap;gap:7px;margin-top:10px;">${QUICK_EXAMPLES.map(v => `<button type="button" class="btn btn-ghost quick-chip" data-brief="${escapeHtml(v)}" style="width:auto;min-height:34px;padding:8px 10px;font-size:12px;">${escapeHtml(v)}</button>`).join('')}</div>
          </div>
          <details style="margin-bottom:20px;">
            ...
            <div class="form-group"><label class="form-label">소소사건 제목</label><input type="text" id="case-title" class="form-input" maxlength="${MAX_TITLE}" placeholder="예: 모기 수면방해 사건"><div class="char-counter"><span id="title-count">0</span>/${MAX_TITLE}</div></div>
            <div class="form-group"><label class="form-label">상세 내용</label><textarea id="case-desc" class="form-textarea" style="min-height:100px;" maxlength="${MAX_DESC}" placeholder="추가 설명이 필요할 때만 입력하세요."></textarea><div class="char-counter"><span id="desc-count">0</span>/${MAX_DESC}</div></div>
          </details>
          <div class="form-group">
          ...
          <div class="form-group">
            <label class="form-label">위원회 성향 <span class="optional">선택 안 하면 랜덤</span></label>
            <div class="judge-grid" id="judge-grid"><div class="judge-option active" data-judge=""><span style="font-size:22px;">🎲</span><div class="judge-option-name">랜덤 배정</div><div class="judge-option-desc">위원회가 알아서 정색</div></div>${JUDGES.map(j => `<div class="judge-option" data-judge="${escapeHtml(j.id)}"><span style="font-size:22px;">${j.icon}</span><div class="judge-option-name">${escapeHtml(j.id)}</div><div class="judge-option-desc">${escapeHtml(j.desc)}</div></div>`).join('')}</div>
          </div>
          <div class="form-group"><label class="form-label">원하는 소소 처분 <span class="optional">선택</span></label><input type="text" id="desired-verdict" class="form-input" maxlength="${MAX_DESIRED}" placeholder="예: 오늘 낮잠 20분 긴급 보장"></div>
          <div class="card" style="padding:14px;margin-bottom:18px;background:rgba(201,168,76,.08);border-color:rgba(201,168,76,.32);"><label style="display:flex;gap:10px;align-items:flex-start;font-size:13px;line-height:1.65;color:var(--cream);cursor:pointer;"><input type="checkbox" id="is-public" checked style="margin-top:4px;"><span><b style="color:var(--gold);">공개 기록에 올리기</b><br><span style="color:var(--cream-dim);">체크하면 다른 유저들이 웃김 점수와 댓글을 남길 수 있습니다.</span></span></label></div>
          <div class="disclaimer" style="margin-bottom:24px;"><strong>⚠️ 접수 전 확인사항</strong><br>· 별것 아닌 소소한 사건일수록 더 재밌습니다.<br>· 실명, 연락처 같은 개인정보는 적지 마세요.<br>· 본 서비스는 AI 기반 오락 콘텐츠이며 실제 효력이 없습니다.</div>
          <button type="submit" class="btn btn-primary" id="submit-btn">긴급속보 내고 처리받기</button>
        </form>
      </div>
    </div>`;
  ...
  const syncAuto = () => {
    ...
    if (!titleInput.dataset.userEdited) titleInput.value = autoTitle(quick.value);
    if (!descInput.dataset.userEdited) descInput.value = compact(quick.value, MAX_DESC);
