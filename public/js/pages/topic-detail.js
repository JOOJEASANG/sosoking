...
export async function renderTopicDetail(container, topicId) {
  ...
  inner.innerHTML = `
    <div class="card topic-detail-card" style="margin-bottom:20px;">
      <span class="topic-card-cat" style="margin-bottom:10px;display:inline-block;">${escHtml(topic.category || '생활')}</span>
      <h2 style="font-family:var(--font-serif);font-size:20px;font-weight:700;color:var(--cream);margin-bottom:8px;line-height:1.4;">${escHtml(topic.title)}</h2>
      <p style="font-size:15px;color:var(--cream-dim);line-height:1.7;margin-bottom:18px;">${escHtml(topic.summary || '')}</p>
      <div class="vs-divider"><span class="vs-text">⚖️ 쟁점 ⚖️</span></div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:14px;">
        <div style="background:rgba(231,76,60,0.08);border:1px solid rgba(231,76,60,0.25);border-radius:10px;padding:14px;">
          <div style="font-size:12px;font-weight:700;color:#e74c3c;margin-bottom:6px;">🔴 원고 입장</div>
          <div style="font-size:14px;color:var(--cream);line-height:1.6;">${escHtml(topic.plaintiffPosition)}</div>
        </div>
        <div style="background:rgba(52,152,219,0.08);border:1px solid rgba(52,152,219,0.25);border-radius:10px;padding:14px;">
          <div style="font-size:12px;font-weight:700;color:#3498db;margin-bottom:6px;">🔵 피고 입장</div>
          <div style="font-size:14px;color:var(--cream);line-height:1.6;">${escHtml(topic.defendantPosition)}</div>
        </div>
      </div>
    </div>

    <div class="submit-topic-tip" style="margin-bottom:20px;">
      🏛️ 이 사건은 <strong>생활법정 형식</strong>으로 진행됩니다.<br>
      원고가 먼저 변론하고, 피고가 반론한 뒤 AI 판사가 판결합니다.
    </div>
    ...
    <div class="form-group">
      ...
      <div class="side-grid">
        <button class="side-btn" data-side="plaintiff">
          ...
          <div class="side-btn-label side-label-p">원고로 변론</div>
        </button>
        <button class="side-btn" data-side="defendant">
          <span class="side-btn-icon">🛡️</span>
          <div class="side-btn-label side-label-d">피고로 반론</div>
        </button>
      </div>
    </div>
    ...
    <div style="background:linear-gradient(135deg,rgba(232,96,44,0.08),rgba(255,138,90,0.06));border:1.5px solid rgba(232,96,44,0.3);border-radius:12px;padding:14px 16px;margin-top:20px;display:flex;align-items:center;gap:12px;">
      ...
      <div style="flex:1;">
        <div style="font-size:13px;font-weight:700;color:var(--gold);margin-bottom:2px;">상대가 아직 없어요!</div>
        <div style="font-size:12px;color:var(--cream-dim);line-height:1.5;">AI 피고/원고와 바로 재판하거나 친구를 초대해보세요</div>
      </div>
    </div>
    ` : ''}

    <div class="form-group" style="margin-top:20px;">
      <label class="form-label">재판 방식</label>
      <div class="mode-grid">
        <button class="mode-btn" data-mode="friend">
          <span class="mode-btn-icon">👫</span>
          <div class="mode-btn-label">친구와 재판</div>
          <div class="mode-btn-desc">링크로 상대 초대</div>
        </button>
        <button class="mode-btn" data-mode="random">
          <span class="mode-btn-icon">${hasRandomOpponent ? '🎲' : '⏳'}</span>
          <div class="mode-btn-label">랜덤 재판</div>
        ...
        ${aiModeEnabled ? `<button class="mode-btn${!hasRandomOpponent ? ' mode-btn-ai-highlight' : ''}" data-mode="ai">
          <span class="mode-btn-icon">🤖</span>
          <div class="mode-btn-label">AI와 재판</div>
    ...
    <div class="form-group" style="margin-top:20px;">
      <label class="form-label">팀 인원 <span style="font-size:10px;color:var(--cream-dim);font-weight:400;text-transform:none;letter-spacing:0;">(각 편당)</span></label>
      <div class="rounds-grid">
        <button class="rounds-btn active" data-team="1"><span class="rounds-btn-num" style="font-size:20px;">1인</span><div class="rounds-btn-label">혼자</div><div class="rounds-btn-desc">1 vs 1</div></button>
        <button class="rounds-btn" data-team="2"><span class="rounds-btn-num" style="font-size:20px;">2인</span><div class="rounds-btn-label">2팀</div><div class="rounds-btn-desc">2 vs 2</div></button>
        <button class="rounds-btn" data-team="3"><span class="rounds-btn-num" style="font-size:20px;">3인</span><div class="rounds-btn-label">3팀</div><div class="rounds-btn-desc">3 vs 3</div></button>
      </div>
    </div>

    <div class="form-group" style="margin-top:20px;">
      <label class="form-label">변론 라운드</label>
      <div class="rounds-grid">
        <button class="rounds-btn" data-rounds="3"><span class="rounds-btn-num">3</span><div class="rounds-btn-label">간단</div><div class="rounds-btn-desc">빠른 판결</div></button>
        <button class="rounds-btn active" data-rounds="5"><span class="rounds-btn-num">5</span><div class="rounds-btn-label">기본</div><div class="rounds-btn-desc">추천</div></button>
        <button class="rounds-btn" data-rounds="7"><span class="rounds-btn-num">7</span><div class="rounds-btn-label">집중</div><div class="rounds-btn-desc">끝장 변론</div></button>
      </div>
    </div>

    <div class="disclaimer" style="margin:20px 0 24px;">
      AI 판결은 실제 법률 판단이 아닙니다.<br>
      재치·공감·유머가 좋은 쪽이 유리합니다.<br>
      <strong>진지하면 질 수 있습니다.</strong>
    </div>

    <button id="start-btn" class="btn btn-primary" disabled>입장과 재판 방식을 선택해주세요</button>
  `;
  ...
  function updateStartBtn() {
    ...
    if (selectedSide && selectedMode) {
      ...
      btn.disabled = true;
      btn.textContent = '입장과 재판 방식을 선택해주세요';
    }
  }
...
function showTeamInviteScreen(container, sessionId, mySide, teamSize, shareToken) {
  ...
  container.innerHTML = `
    <div style="text-align:center;padding:40px 0 20px;">
      ...
      <h2 style="font-family:var(--font-serif);font-size:22px;font-weight:700;color:var(--gold);margin-bottom:8px;">팀 재판 생성 완료!</h2>
      <p style="font-size:14px;color:var(--cream-dim);line-height:1.7;margin-bottom:28px;">팀원을 초대하거나 바로 재판장으로 이동하세요.</p>
    </div>
    ...
    <div class="card" style="margin-bottom:24px;">
      <div style="font-size:11px;font-weight:700;color:var(--cream-dim);letter-spacing:.08em;margin-bottom:10px;">상대편 초대 링크</div>
      <button id="copy-opp-link" class="btn btn-ghost" style="margin-bottom:0;">🔗 상대방 초대 링크 복사</button>
    </div>
    ` : ''}

    <a href="#/debate/${sessionId}" class="btn btn-primary">⚖️ 재판장으로 이동</a>
  `;
