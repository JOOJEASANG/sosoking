  ...
  container.innerHTML = `
    <main class="hunt-play-page">
      ...
      <section class="hunt-briefing">
        ...
        <div class="escape-plan"><b>숨겨진 도주 시나리오</b><span>AI는 이미 빠져나갈 계획을 세웠습니다. 출입기록, 시간대, 진술의 빈틈을 압박하세요.</span></div>
      </section>
...
function bind(container){
  ...
  container.querySelectorAll('[data-question]').forEach(btn=>btn.addEventListener('click',()=>{ const r=interrogate(Number(btn.dataset.question)); if(!r)return; const log=document.getElementById('suspect-log'); if(log) log.innerHTML = `<p><b>${esc(r.speaker)}:</b> “${esc(r.line)}”</p>`; refreshMeters(r.game); }));
