...
function bind(container){
  ...
  document.getElementById('attempt-arrest')?.addEventListener('click', async()=>{
    const btn = document.getElementById('attempt-arrest');
    if (btn) { btn.disabled = true; btn.textContent = '수사망 검증 중...'; }
    const localResult = attemptArrest([...arrestEvidence]);
    if(!localResult){ if(btn){btn.disabled=false;btn.textContent='🚨 검거 시도';} return; }
    try {
      const game = getGame();
      const fn = httpsCallable(functions, 'attemptAiHuntArrest');
      const res = await fn({
        caseId: game.remoteCaseId || game.caseId,
        selectedEvidence: [...arrestEvidence],
      ...
      localStorage.setItem(RESULT_KEY, JSON.stringify(localResult));
    }
    location.hash='#/hunt/result';
  });
}
