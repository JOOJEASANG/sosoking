import { renderSubmit as renderBaseSubmit } from './submit.js?v=20260702-21';

function lvLabel(v) {
  const n = Number(v || 5);
  if (n <= 2) return '먼지급 해프닝';
  if (n <= 4) return '피식 재판감';
  if (n <= 6) return '소소재판 개정';
  if (n <= 8) return '재판부 긴급입장';
  return '대형 소소재판';
}
...
function ensureGameStyle() {
  ...
  style.textContent = `
    ...
    .judge-option::after{content:'재판부 카드';position:absolute;right:9px;top:8px;font-size:9px;color:var(--text-soft,var(--cream-dim));font-weight:900;letter-spacing:.08em;}
...
function decorateSubmit(container) {
  ...
  if (topCard && !document.getElementById('court-submit-docket')) {
    ...
    topCard.insertAdjacentHTML('beforeend', `<div id="court-submit-docket" class="court-ledger"><div><strong>한 줄</strong><span>소장 접수</span></div><div><strong>재판</strong><span>과한 심리</span></div><div><strong>웃김</strong><span>점수 랭킹</span></div></div>`);
  }
  const rangeGroup = document.getElementById('grievance')?.closest('.form-group');
  if (rangeGroup && !document.getElementById('game-lv-card')) {
    rangeGroup.insertAdjacentHTML('afterbegin', `<div id="game-lv-card" class="game-lv-card"><div class="game-lv-head"><div class="game-lv-title">사소함 레벨</div><div class="game-lv-num">Lv.<span id="game-lv-num">5</span></div></div><div class="game-lv-bar"><div class="game-lv-fill" id="game-lv-fill"></div></div><div class="game-lv-caption" id="game-lv-caption">소소재판 개정</div></div>`);
    const input = document.getElementById('grievance');
    const sync = () => { const v = Number(input.value || 5); document.getElementById('game-lv-num').textContent = v; document.getElementById('game-lv-fill').style.width = `${v * 10}%`; document.getElementById('game-lv-caption').textContent = lvLabel(v); };
  ...
  if (form && !document.getElementById('court-submit-flow')) {
    form.insertAdjacentHTML('afterbegin', `<div id="court-submit-flow" class="court-document" style="padding:16px;margin-bottom:18px;"><div class="court-kicker">SOSO TRIAL + FUN SCORE</div><div class="court-title" style="font-size:19px;">소소한 한 줄이 재판과 판결문이 됩니다</div><div class="court-timeline"><div class="court-step"><div class="court-step-num">1</div><div><div class="court-step-title">한 줄 소장 접수</div><div class="court-step-text">모기, 엘리베이터, 라면, 만두 같은 사안을 짧게 던집니다.</div></div></div><div class="court-step"><div class="court-step-num">2</div><div><div class="court-step-title">과한 재판 개정</div><div class="court-step-text">별일 아닌 일이 재판부 앞에서 괜히 중대해집니다.</div></div></div><div class="court-step"><div class="court-step-num">3</div><div><div class="court-step-title">웃김점수 경쟁</div><div class="court-step-text">유저들이 1~10점으로 평가하고 소소킹 후보를 정합니다.</div></div></div></div></div>`);
  }
}
export async function renderSubmit(container) { await renderBaseSubmit(container); decorateSubmit(container); }
