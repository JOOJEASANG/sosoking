import { renderSubmit as renderBaseSubmit } from './submit.js?v=20260708-title2';
...
function bindSmartAutoTitle(container) {
  ...
  title.placeholder = '예: 공원에서 산책중이던 리트리버가 내 빵을 먹은 사건';
  desc.placeholder = '예: 공원에서 빵을 먹고 있었는데 한눈판 사이 산책중이던 리트리버가 내 빵을 먹었어요.';

  let userEdited = title.value.trim().length > 0;
  title.insertAdjacentHTML('afterend', '<div id="auto-title-note" class="auto-title-note">사건명을 비워두면 사건 내용에서 장소·행위자·대상·행동을 뽑아 자동 정리합니다.</div>');

  title.addEventListener('input', () => {
    userEdited = title.value.trim().length > 0;
    if (userEdited) title.dataset.autoGeneratedTitle = '';
  });
  desc.addEventListener('input', () => {
    if (userEdited) return;
    const next = smartTitle(desc.value);
    title.value = next;
    title.dataset.autoGeneratedTitle = next ? '1' : '';
