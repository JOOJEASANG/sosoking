/* ranking.js — 토론실 목록·상세·사용자 등록·A/B 투표·이미지 */
...
function ensureStyle() {
  ...
  style.textContent = `
    ...
    .debate-compose{display:none}.debate-compose.open{display:block}.debate-write{display:grid;gap:12px}.debate-write-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.debate-write label{display:grid;gap:6px;color:var(--color-text-primary);font-size:12px;font-weight:900}.debate-write input,.debate-write textarea{width:100%;font:inherit}.debate-write textarea{min-height:96px;resize:vertical}.debate-write-side{border:1px solid rgba(100,116,139,.14);border-radius:18px;padding:13px}.debate-write-side strong{display:block;margin-bottom:10px;color:var(--color-text-primary)}.debate-write-help{font-size:11px;color:var(--color-text-muted);line-height:1.6}.debate-image-picker{border:1px dashed rgba(224,93,68,.38);border-radius:18px;padding:14px;background:rgba(224,93,68,.045)}.debate-image-preview{display:none;margin-top:10px;grid-template-columns:128px minmax(0,1fr);gap:12px;align-items:center}.debate-image-preview.open{display:grid}.debate-image-preview img{width:128px;height:96px;object-fit:cover;border-radius:14px;background:#e2e8f0}.debate-image-preview__info{display:grid;gap:7px}
    @media(max-width:900px){.debate-body{grid-template-columns:1fr}.debate-side{position:static}.debate-card,.debate-card.no-image{grid-template-columns:82px 44px minmax(0,1fr)}.debate-stats{grid-column:3;justify-content:flex-start}.debate-card__image{width:82px;height:68px}}
    @media(max-width:760px){.debate-card,.debate-card.no-image{grid-template-columns:44px minmax(0,1fr)}.debate-card__image{grid-column:1/-1;width:100%;height:150px}.debate-rank{width:42px;height:42px;border-radius:14px}.debate-stats{grid-column:2;justify-content:flex-start}.debate-write-grid{grid-template-columns:1fr}.debate-vote{gap:8px}.debate-vote button{min-height:154px;padding:13px}.debate-vote__letter{width:36px;height:36px;font-size:18px}.debate-choice-head{align-items:flex-start;flex-direction:column}.debate-image-preview{grid-template-columns:92px minmax(0,1fr)}.debate-image-preview img{width:92px;height:76px}}
  `;
...
async function loadDebateList(element, order = 'latest') {
  ...
  const items = await attachImages('debate', Array.isArray(result.debates) ? result.debates : []);
...
function bindDebateComposer(element) {
  ...
  const input = element.querySelector('#debate-write-image');
  const preview = element.querySelector('#debate-image-preview');
  const previewImage = element.querySelector('#debate-image-preview-img');
  const meta = element.querySelector('#debate-image-meta');
  const status = element.querySelector('#debate-image-status');
  ...
  element.querySelector('#debate-image-remove')?.addEventListener('click', () => {
    releasePreparedImage(preparedImage);
    preparedImage = null;
    input.value = '';
    preview?.classList.remove('open');
  ...
  element.querySelector('#debate-write-submit')?.addEventListener('click', async event => {
    ...
    try {
      if (preparedImage) {
        const uploaded = await uploadPreparedImage(preparedImage, 'debate');
        ...
        button.textContent = '토론 등록 중…';
      }
      const result = await call('createUserDebate', payload);
      if (!result.ok || !result.id) throw result.error || new Error('토론 등록에 실패했습니다.');
      toast.success('토론을 등록했습니다.');
      navigate(`/debate/${result.id}`);
    } catch (error) {
      button.disabled = false;
      button.textContent = '토론 등록하기';
      toast.error(error.message || '토론 등록에 실패했습니다.');
    }
  });
}
...
export async function renderDebateDetail(id) {
  ...
  const [debate] = await attachImages('debate', [result.debate]);
  ...
  element.innerHTML = `<div class="debate-page page-enter"><section class="debate-hero"><button class="btn btn--ghost btn--sm" id="debate-back">← 토론실</button><div class="debate-eyebrow" style="margin-top:18px">${typeLabel}</div><h1>${escHtml(debate.title)}</h1><p>${escHtml(debate.summary)}</p><div class="debate-tags" style="margin-top:13px"><span class="debate-chip">${escHtml(debate.category)}</span>${tags.map(tag => `<span class="debate-chip">#${escHtml(tag)}</span>`).join('')}</div></section>${imageMarkup(debate, 'debate-cover', debate.title)}<div class="debate-body"><div class="debate-main"><section class="debate-panel"><h2>상황 보기</h2><div class="debate-context">${context.map(item => `<div class="debate-context__item">${escHtml(item)}</div>`).join('')}</div></section><section class="debate-panel"><div class="debate-choice-head"><h2>내 선택은 여기에</h2><div class="debate-choice-status">현재 선택 <strong>${choice}</strong></div></div><div class="debate-vote"><button data-vote="agree" class="${result.myVote === 'agree' ? 'active' : ''}"><span class="debate-vote__letter">A</span>${result.myVote === 'agree' ? '<span class="debate-vote__selected">내 선택</span>' : ''}<div class="debate-vote__label"><span>${escHtml(debate.agreeTitle)}</span><span class="debate-vote__count">${Number(debate.agreeCount || 0)}표</span></div><p>${escHtml(debate.agreeText)}</p></button><button data-vote="disagree" class="${result.myVote === 'disagree' ? 'active' : ''}"><span class="debate-vote__letter">B</span>${result.myVote === 'disagree' ? '<span class="debate-vote__selected">내 선택</span>' : ''}<div class="debate-vote__label"><span>${escHtml(debate.disagreeTitle)}</span><span class="debate-vote__count">${Number(debate.disagreeCount || 0)}표</span></div><p>${escHtml(debate.disagreeText)}</p></button></div></section><section class="debate-panel"><h2>댓글 토론</h2><div class="debate-form"><select id="debate-comment-side"><option value="neutral">중립 의견</option><option value="agree">A 의견</option><option value="disagree">B 의견</option></select><textarea id="debate-comment-text" maxlength="700" placeholder="왜 A 또는 B라고 생각하는지 의견을 적어주세요."></textarea><button class="btn btn--primary" id="debate-comment-submit">의견 등록</button></div><div id="debate-comments" style="margin-top:14px"></div></section></div><aside class="debate-side"><section class="debate-panel"><h2>현재 투표</h2><div class="debate-meter"><div class="debate-meter__row"><span>A ${escHtml(debate.agreeTitle)}</span><div class="debate-meter__bar"><span style="width:${agreePercent}%"></span></div><b>${agreePercent}%</b></div><div class="debate-meter__row"><span>B ${escHtml(debate.disagreeTitle)}</span><div class="debate-meter__bar"><span style="width:${disagreePercent}%"></span></div><b>${disagreePercent}%</b></div></div><p style="margin-top:12px">총 ${total}표 · 댓글 ${Number(debate.commentCount || 0)}개</p></section><section class="debate-panel"><h2>생각해볼 질문</h2>${questions.length ? questions.map(question => `<p>• ${escHtml(question)}</p>`).join('') : '<p>자유롭게 의견을 나눠보세요.</p>'}</section><section class="debate-panel"><h2>자료가 필요하다면</h2><p>생활정보와 정리 자료는 자료실에서 확인하고 댓글도 남길 수 있습니다.</p><button class="btn btn--ghost" id="go-materials">자료실 열기</button></section></aside></div></div>`;
