...
function renderListMarkup(container, syncing = false) {
  ...
  container.innerHTML = `<main class="predict-app simple-page"><div class="simple-header"><a href="#/" class="back-link">‹</a><div><span>${syncing ? '서버 동기화 중...' : '오늘의 예측판'}</span><h1>핫이슈의 내일을 맞혀라</h1></div><b>${wallet.balance.toLocaleString()}</b></div><section class="board-preview-section list-mode"><div class="board-list">${boards.map(board => boardCard(board)).join('')}</div></section></main>`;
}
...
function renderDetailMarkup(container, boardId, syncing = false) {
  ...
  container.innerHTML = `
    <main class="predict-app simple-page">
      ...
      <section class="comments-section"><div class="section-head compact"><div><span>COMMENTS</span><h2>예측 근거</h2></div></div><form id="comment-form" class="comment-form"><input id="comment-input" maxlength="120" placeholder="내 생각엔 이쪽이 맞을 것 같은데..." /><button>등록</button></form><div class="comment-list">${comments.map(c => `<div class="comment-item ${c.mine ? 'mine' : ''}"><b>${escapeHtml(c.side)}</b><p>${escapeHtml(c.text)}</p><small>좋아요 ${Number(c.likes || 0)}</small></div>`).join('')}</div></section>
    </main>`;
