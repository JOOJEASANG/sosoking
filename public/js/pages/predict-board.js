    ...
    <main class="predict-app simple-page predict-list-page">
      <section class="predict-list-hero">
        ...
        <div class="list-hero-copy">
          ...
          <h1>오늘의 3판,<br><em>무엇을 맞힐까?</em></h1>
          <p>각 예측판은 먼저 ‘예측 대상’을 보여주고, 그다음 내일 확인할 질문과 정답 기준을 보여줍니다. 어떤 이슈를 말하는지 헷갈리지 않게 정리했습니다.</p>
          <div class="list-hero-badges"><b>🔥 최고 관심도 ${Number(hottest?.heat || 0)}</b><b>🎯 예측 대상 표시</b><b>📌 정답 기준 확인</b></div>
        </div>
      ...
      <section class="pick-briefing">
        <article><span>01</span><b>대상 확인</b><p>무엇에 대한 예측인지 먼저 봅니다.</p></article>
        <article><span>02</span><b>질문 확인</b><p>내일 어떤 결과를 맞히는지 봅니다.</p></article>
        <article><span>03</span><b>기준 확인</b><p>무엇을 정답으로 볼지 확인합니다.</p></article>
      </section>
      <section class="board-preview-section list-mode upgraded-list-mode">
        <div class="section-head list-section-head"><div><span>CHOOSE YOUR ISSUE</span><h2>예측할 대상을 선택하세요</h2></div><p>${syncing ? '최신 예측판을 불러오는 중입니다.' : '대상·질문·정답 기준이 명확한 판만 참여하세요.'}</p></div>
  ...
  container.innerHTML = `
    <main class="predict-app simple-page">
      ...
      <section class="detail-layout">
        <article class="detail-main-card clear-prediction-card">
          <div class="heat-row"><span>🔥 관심도 ${Number(board.heat || 0)}</span><span>참여 ${Number(board.participants || 0).toLocaleString()}</span></div>
          ${targetPanel(board)}
          <div class="question-box"><span>맞혀야 하는 질문</span><h2>${escapeHtml(clearQuestion(board))}</h2><p>${escapeHtml(board.summary)}</p></div>
          ${resultNotice}
          ${moodPanel(board)}
      ...
      <section class="comments-section"><div class="section-head compact"><div><span>REASONS</span><h2>예측 근거 한 줄</h2></div><p class="comment-guide">사람들이 왜 그쪽을 골랐는지 보는 곳입니다. 짧고 명확할수록 좋아요.</p></div><form id="comment-form" class="comment-form"><input id="comment-input" maxlength="120" placeholder="예: ${escapeAttr(reasonPlaceholder(board))}" /><button>등록</button></form><div class="comment-list">${comments.length ? comments.map(c => `<div class="comment-item ${c.mine ? 'mine' : ''}"><b>${escapeHtml(c.side)}</b><p>${escapeHtml(c.text)}</p><small>공감 ${Number(c.likes || 0)}</small></div>`).join('') : `<div class="comment-item"><b>아직 근거 없음</b><p>첫 번째 근거를 남겨보세요.</p><small>공감 0</small></div>`}</div></section>
    </main>`;

  document.getElementById('prediction-form')?.addEventListener('submit', async (event) => {
    event.preventDefault();
    const btn = event.currentTarget.querySelector('.submit-prediction');
...
function targetPanel(board) {
  return `<section class="prediction-target-panel"><span>이번 판의 예측 대상</span><h2>${escapeHtml(issueTarget(board))}</h2><p>${escapeHtml(targetDescription(board))}</p><div><b>${escapeHtml(board.category || '이슈')}</b><b>마감 ${escapeHtml(board.closeAt || '오늘')}</b><b>결과 ${escapeHtml(board.resultAt || '내일')}</b></div></section>`;
}

function issueTarget(board) {
  return board.issue || board.subject || board.keyword || board.title || '오늘의 이슈';
}

function targetDescription(board) {
  if (board.issue && board.title && board.issue !== board.title) return board.title;
  return '아래 질문은 이 예측 대상을 기준으로 판정됩니다.';
}

function clearQuestion(board) {
  const q = board.question || '내일도 계속 화제가 될까요?';
  const target = issueTarget(board);
  const normalizedQ = q.replace(/\s/g, '');
  const normalizedTarget = target.replace(/\s/g, '').slice(0, 8);
  if (normalizedTarget && normalizedQ.includes(normalizedTarget)) return q;
  return `「${target}」 기준으로 ${q}`;
}

function reasonPlaceholder(board) {
  const target = issueTarget(board).slice(0, 18);
  return `${target}은 내일까지 관심이 이어질 듯`;
}
...
function renderForm(board) {
  ...
  return `<form id="prediction-form" class="prediction-form"><div class="panel-title">내 선택</div><p class="panel-help"><b>${escapeHtml(issueTarget(board))}</b>에 대한 내일의 결과를 하나 고르세요.</p><div class="option-list">${options.map((option, index) => `<label class="option-card"><input type="radio" name="predict-option" value="${escapeAttr(option.id)}" ${index === 0 ? 'checked' : ''}><span><b>${escapeHtml(option.label)}</b><small>예상 배율 x${Number(option.odds || 1)}</small></span></label>`).join('')}</div><label class="input-label">사용할 소소머니</label><select id="predict-amount"><option value="500">500</option><option value="1000" selected>1,000</option><option value="3000">3,000</option><option value="5000">5,000</option></select><label class="input-label">예측 근거 한 줄</label><textarea id="predict-comment" maxlength="120" placeholder="${escapeAttr(reasonPlaceholder(board))}"></textarea><button class="submit-prediction">내 예측 등록</button><p class="money-note">소소머니는 게임 전용 포인트이며 현금 가치가 없습니다.</p></form>`;
}
...
function boardCard(board, index = 0) {
  ...
  return `<a class="board-card upgraded-board-card clear-board-card" href="#/predict/${encodeURIComponent(board.id)}"><div class="board-rank">0${index + 1}</div><div class="board-card-top"><span>${escapeHtml(board.category)}</span><b>🔥 ${Number(board.heat || 0)}</b></div><div class="board-target"><span>예측 대상</span><strong>${escapeHtml(issueTarget(board))}</strong></div><div class="board-question"><span>질문</span><b>${escapeHtml(clearQuestion(board))}</b></div><p>${escapeHtml(board.summary)}</p><div class="board-mini-tags"><span>참여 ${Number(board.participants || 0).toLocaleString()}</span><span>${statusText}</span></div><div class="board-card-cta"><b>자세히 보고 예측하기</b><span>›</span></div></a>`;
}
