...
export async function renderDetail(id) {
  ...
  try {
    ...
    renderDetailPage(el, post, comments);
  } catch (e) {
    console.error(e);
...
function renderDetailPage(el, post, comments) {
      ...
      return `
        <div class="quiz-box" id="quiz-area">
          ...
          <div id="quiz-result" style="display:none" class="quiz-result">
            ...
            ${post.explanation ? `<div style="margin-top:8px;font-size:13px;color:var(--color-text-secondary)">💡 ${escHtml(post.explanation)}</div>` : ''}
          </div>
        </div>`;
        ...
        ? `<div style="padding:16px;background:#F3F4F6;border-radius:10px;margin-top:8px">
            ...
            ${[...post.keyword].map(ch => `
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
                ...
                <span style="font-size:13px;color:var(--color-text-muted)">:  댓글로 참여해보세요</span>
              </div>`).join('')}
          </div>`
        : '';
  ...
  if (post.type === 'acrostic' && post.keyword) {
    ...
    return `
      <div style="padding:20px">
        ...
        <button class="btn btn--primary btn--sm" id="btn-acrostic-submit" style="margin-top:8px">삼행시 댓글로 올리기</button>
      </div>`;
  }
  return '';
}
...
function setupDetailEvents(post, el) {
  ...
  document.querySelectorAll('[data-answer]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      resultEl.querySelector('.quiz-result__icon').textContent = correct ? '⭕' : '❌';
      resultEl.querySelector('.quiz-result__text').textContent = correct ? '정답이에요!' : '오답이에요!';
  ...
  document.querySelectorAll('[data-quiz-idx]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const correct = idx === post.answerIdx;
      const resultEl = document.getElementById('quiz-result');
      if (resultEl) {
        resultEl.style.display = '';
        resultEl.className = `quiz-result quiz-result--${correct ? 'correct' : 'wrong'}`;
        resultEl.innerHTML = `
          ...
          <div class="quiz-result__text">${correct ? '정답이에요!' : '오답이에요!'}</div>
          ${post.explanation ? `<div style="margin-top:8px;font-size:13px">💡 ${escHtml(post.explanation)}</div>` : ''}`;
      }
  ...
  document.getElementById('btn-quiz-submit')?.addEventListener('click', () => {
    ...
    if (!answer) return;
    const correct = answer === post.answer;
    const resultEl = document.getElementById('quiz-result');
    if (resultEl) {
      resultEl.style.display = '';
      resultEl.className = `quiz-result quiz-result--${correct ? 'correct' : 'wrong'}`;
      resultEl.innerHTML = `<div class="quiz-result__icon">${correct ? '⭕' : '❌'}</div><div class="quiz-result__text">${correct ? '정답이에요!' : '오답이에요!'}</div>`;
    }
  ...
  document.getElementById('btn-acrostic-submit')?.addEventListener('click', async () => {
    ...
    const text = [...post.keyword].map((ch, i) => `${ch}: ${lines[i]}`).join('\n');
    try {
      await addDoc(collection(db, 'feeds', post.id, 'comments'), {
        text,
        authorId:   auth.currentUser.uid,
