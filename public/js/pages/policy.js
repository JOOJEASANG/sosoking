...
export const POLICY_EFFECTIVE_DATE = '2026년 9월 11일';
...
제1조 (목적)
본 약관은 소소킹(이하 “서비스”)가 제공하는 오락형 AI 생활판결(판결소), 미친 상담소, 미친 번역소, 공개 판결기록, 민심소 투표·토론, 좋아요, 명예의 전당 및 관련 기능의 이용 조건과 운영 원칙을 정합니다.
...
1. 미친 상담소는 이용자가 입력한 고민을, 미친 번역소는 이용자가 입력한 문장을 바탕으로 생성형 AI가 병맛·과장된 오락 결과를 생성하는 기능입니다.
2. 판결소 사건과 달리 미친 상담소·미친 번역소의 결과는 생성 즉시 공개되어 공개 목록과 검색엔진에 노출될 수 있으며, 다른 이용자가 좋아요를 남길 수 있습니다. 따라서 실명·연락처·주소 등 개인정보나 민감한 내용을 입력해서는 안 됩니다.
3. 상담소의 처방과 번역소의 번역도 오락 콘텐츠이며 실제 의학·심리 상담, 번역 서비스 또는 전문가 의견이 아닙니다.
4. 상담소·번역소·좋아요 등 부가 기능의 이용 횟수는 비로그인(익명)·회원 여부에 따라 운영 설정으로 제한될 수 있습니다.
...
- 참여 정보: 민심소 투표, 반응, 공개 댓글·토론 내용, 좋아요 기록, 신고 내용과 처리 상태
- 부가 서비스 정보: 미친 상담소에 입력한 고민, 미친 번역소에 입력한 문장, AI가 생성한 처방·번역 결과
...
- 판결소 사건과 달리 미친 상담소·미친 번역소의 결과는 기본적으로 공개되어 공개 목록·검색엔진에 노출될 수 있고 좋아요 수가 함께 표시됩니다. 입력 전 개인정보가 포함되지 않았는지 반드시 확인하세요.

5. 생성형 AI 및 외부 서비스 이용
- AI 판결, 상담소 처방, 번역소 번역 생성에는 Google Gemini API가 사용될 수 있으며, 이용자가 입력한 사건·고민·문장 내용은 결과 생성을 위해 필요한 범위에서 AI 제공자에게 전송될 수 있습니다.
...
9. 미친 상담소·미친 번역소도 오락입니다
미친 상담소의 처방과 미친 번역소의 번역도 AI가 재미로 만든 오락 콘텐츠이며, 실제 의학·심리 상담이나 번역 서비스가 아닙니다. 판결소 사건과 달리 이 결과는 기본적으로 공개되어 공개 목록·검색엔진에 노출될 수 있으니, 실명·연락처 등 개인정보나 민감한 내용은 입력하지 마세요.`
};
