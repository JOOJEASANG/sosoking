...
② 이용자는 하루 최대 10회 판정 요청 및 5회 주제 등록이 가능합니다.
...
소소킹 토론배틀은 일상의 사소한 갈등을 소재로 친구와 토론하고, AI 심판에게 재밌는 판정을 받아보는 엔터테인먼트 서비스입니다.

실제 법원 판결, 법률 자문, 분쟁 조정 서비스가 아닙니다. 판정 결과를 실제 생활의 법적 근거로 사용하지 마세요.


2. 사용 AI 모델

본 서비스는 Google의 Gemini 2.5 Flash 모델을 사용합니다. AI는 입력된 사건 내용을 바탕으로 오락용 판정문을 생성하며, 법률 전문 지식을 기반으로 하지 않습니다.
...
· 판정 결과를 타인에게 공유할 때는 오락 목적 서비스임을 명시해주세요.
...
이용자가 입력한 사건 내용은 AI 판정 생성 목적으로 Google Gemini API에 전송됩니다. 처리 후 내용은 Google의 데이터 정책에 따라 관리됩니다.
자세한 내용: https://policies.google.com/privacy


6. 책임 제한

운영자는 AI가 생성한 판정 내용의 정확성을 보증하지 않으며, 판정 결과를 실생활에 적용함으로써 발생하는 어떠한 결과에 대해서도 책임을 지지 않습니다.`
};
