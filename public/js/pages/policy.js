...
- 보안·운영 정보: 접수 및 AI 요청 횟수, 처리 시각, 오류·사용량 기록
사건 내용에는 실명, 연락처, 주소, 계좌번호, 주민등록번호 등 타인을 식별할 수 있는 정보를 입력하지 않아야 합니다.

2. 처리 목적
- 회원 로그인, 이메일 인증, 닉네임 중복 방지 및 내 사건 관리
- 입력 내용을 바탕으로 한 AI 판결문 생성
- 이용자가 명시적으로 선택한 공개 판결기록 제공
- 투표·댓글·신고 기능 운영
- 개인정보·고위험 내용·시스템 공격성 문장의 사전 탐지
- 중복 요청, 비정상 이용 및 AI 비용 남용 방지

3. 공개 범위
사건과 판결문은 기본적으로 비공개입니다. 이용자가 사건 접수 화면의 공개 동의 항목을 직접 선택하거나 결과 화면에서 공개로 전환한 경우에만 닉네임, 사건 내용 및 AI 판결문이 공개 판결기록에 표시됩니다. 인증 UID와 이메일 주소는 공개 판결문 및 공개 댓글에 포함하지 않습니다. 이용자는 결과 화면에서 다시 비공개로 전환할 수 있습니다.
...
5. 보유 및 삭제
회원 정보, 사건 및 판결기록은 서비스 제공과 내 사건 관리를 위해 보관됩니다. 공개 여부는 이용자가 직접 변경할 수 있으며, 계정·사건·댓글 등 저장 정보의 열람 또는 삭제 요청은 사이트 하단의 운영자 연락처로 문의할 수 있습니다. 법령상 보존 의무나 분쟁 대응 필요가 있는 경우 해당 기간 동안 보관될 수 있습니다.

6. 안전조치
Firestore 보안 규칙, 서버 전용 쓰기, 이메일 인증, 요청 횟수 제한을 적용합니다. 사건·항소 이유·공개 댓글은 저장 또는 AI 전송 전에 개인정보와 고위험 표현 등을 자동 검사하며, 해당 내용은 접수가 제한될 수 있습니다. App Check가 활성화된 경우 비정상 클라이언트 요청을 확인하기 위해 reCAPTCHA 기반 토큰이 처리될 수 있습니다.
...
4. 사용 AI 모델
본 서비스는 운영 설정에 따라 Google Gemini 계열 모델을 사용하며, 장애 시 로컬 대체 문서를 생성할 수 있습니다.
...
export async function renderPolicy(container, type) {
  ...
  try {
    ...
    const obsoletePrivacy = safeType === 'privacy'
      && saved.includes('개인 식별 정보를 수집하지 않습니다');
    const raw = saved && !obsoletePrivacy
      ? saved
