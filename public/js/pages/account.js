...
function providerLabel(user) {
  ...
  if (providers.includes('google.com')) return 'Google 계정';
  if (providers.includes('password')) return '이메일 계정';
  return user?.isAnonymous ? '게스트' : '계정';
}
...
export async function renderAccount(container) {
  ...
  if (!user || user.isAnonymous) {
    container.innerHTML = `<main class="predict-app account-page"><section class="account-hero"><a href="#/" class="back-link">‹</a><div class="account-hero-copy"><img src="/logo.svg" alt="소소킹"><div><span>ACCOUNT</span><h1>로그인이 필요합니다</h1><p>내정보 수정은 로그인 후 사용할 수 있습니다.</p></div></div><b>🔐</b></section><section class="account-card empty"><p>계정을 연결하면 닉네임, 비밀번호, 예측 기록을 관리할 수 있습니다.</p><a href="#/login">로그인하기</a></section></main>`;
    return;
  }
  ...
  container.innerHTML = `
    <main class="predict-app account-page">
      <section class="account-hero">
        ...
        <div class="account-hero-copy"><img src="/logo.svg" alt="소소킹"><div><span>MY INFO</span><h1>내정보 수정</h1><p>소소킹에서 표시될 닉네임과 계정 보안 정보를 관리합니다.</p></div></div>
        <b>⚙️</b>
      </section>
      ...
      <section class="account-layout">
        <aside class="account-summary-card">
          ...
          <b>${escapeHtml(nickname || '소소킹 유저')}</b>
          <span>${escapeHtml(providerLabel(user))}</span>
          <p>${email ? escapeHtml(email) : '이메일 정보 없음'}</p>
          <a href="#/history">내 기록 보기</a>
        </aside>

        <div class="account-panels">
          <form id="nickname-form" class="account-card">
            <div class="account-card-head"><span>NICKNAME</span><h2>닉네임 변경</h2><p>예측 근거, 소소피드, 댓글에 표시되는 이름입니다.</p></div>
            <label>현재 닉네임</label>
            <input value="${escapeAttr(nickname || '')}" disabled>
            <label>새 닉네임</label>
            <div class="account-row"><input id="new-nickname" maxlength="12" placeholder="닉네임 2~12자"><button id="check-nickname" type="button">중복확인</button></div>
            <div id="nickname-status" class="account-status"></div>
            <button id="save-nickname" type="submit" disabled>닉네임 저장</button>
          </form>

          <form id="password-form" class="account-card">
            <div class="account-card-head"><span>PASSWORD</span><h2>비밀번호 변경</h2><p>${canChangePassword ? '현재 비밀번호 확인 후 새 비밀번호로 변경합니다.' : 'Google 계정은 Google 계정 관리에서 비밀번호를 변경해야 합니다.'}</p></div>
            ${canChangePassword ? `<label>현재 비밀번호</label><input id="current-password" type="password" autocomplete="current-password" placeholder="현재 비밀번호"><label>새 비밀번호</label><input id="new-password" type="password" autocomplete="new-password" placeholder="새 비밀번호 6자 이상"><label>새 비밀번호 확인</label><input id="new-password2" type="password" autocomplete="new-password" placeholder="새 비밀번호 확인"><button type="submit">비밀번호 변경</button>` : `<div class="account-notice">현재 계정은 ${escapeHtml(providerLabel(user))}입니다. 사이트 안에서는 비밀번호를 직접 변경할 수 없습니다.</div>`}
          </form>

          <form id="reset-form" class="account-card">
            <div class="account-card-head"><span>RESET</span><h2>비밀번호 재설정 메일</h2><p>이메일 계정 비밀번호를 잊은 경우 재설정 메일을 받을 수 있습니다.</p></div>
            <label>이메일</label><input id="reset-email" type="email" value="${escapeAttr(email)}" placeholder="가입 이메일">
            <button type="submit">재설정 메일 보내기</button>
          </form>

          <section class="account-card danger">
            <div class="account-card-head"><span>LOGOUT</span><h2>로그아웃</h2><p>로그아웃 후에는 게스트 상태로 전환됩니다.</p></div>
  ...
  let checkedNickname = '';
  container.querySelector('#new-nickname')?.addEventListener('input', () => {
    checkedNickname = '';
    container.querySelector('#save-nickname').disabled = true;
    container.querySelector('#nickname-status').textContent = '';
  });

  container.querySelector('#check-nickname')?.addEventListener('click', async () => {
    const nick = container.querySelector('#new-nickname').value.trim();
    const status = container.querySelector('#nickname-status');
    const invalid = validNickname(nick);
    if (invalid) { status.className = 'account-status error'; status.textContent = invalid; return; }
    if (nick === nickname) { status.className = 'account-status error'; status.textContent = '현재 닉네임과 같습니다.'; return; }
    status.className = 'account-status'; status.textContent = '확인 중...';
    try {
      const snap = await getDoc(doc(db, 'nicknames', nick));
      if (snap.exists()) { status.className = 'account-status error'; status.textContent = '이미 사용 중인 닉네임입니다.'; return; }
      checkedNickname = nick;
      status.className = 'account-status ok'; status.textContent = '사용 가능한 닉네임입니다.';
      container.querySelector('#save-nickname').disabled = false;
    } catch {
      status.className = 'account-status error'; status.textContent = '중복확인에 실패했습니다.';
    }
  });

  container.querySelector('#nickname-form')?.addEventListener('submit', async e => {
    e.preventDefault();
    const nick = container.querySelector('#new-nickname').value.trim();
    if (!checkedNickname || checkedNickname !== nick) { showToast('닉네임 중복확인을 먼저 해주세요.', 'error'); return; }
    const btn = container.querySelector('#save-nickname');
    btn.disabled = true; btn.textContent = '저장 중...';
    try {
      await setDoc(doc(db, 'nicknames', nick), { uid: user.uid, createdAt: serverTimestamp() });
      await setDoc(doc(db, 'users', user.uid), { nickname: nick, nicknameUpdatedAt: serverTimestamp() }, { merge: true });
      if (nickname && nickname !== nick) await deleteDoc(doc(db, 'nicknames', nickname)).catch(() => {});
  ...
  container.querySelector('#password-form')?.addEventListener('submit', async e => {
    ...
    try { await changeEmailPassword(current, next); showToast('비밀번호가 변경되었습니다.', 'success'); e.currentTarget.reset(); }
    catch (err) { showToast(err.message || '비밀번호 변경에 실패했습니다.', 'error'); }
    finally { btn.disabled = false; btn.textContent = '비밀번호 변경'; }
  });

  container.querySelector('#reset-form')?.addEventListener('submit', async e => {
    e.preventDefault();
    const emailValue = container.querySelector('#reset-email').value.trim();
    const btn = e.currentTarget.querySelector('button');
    btn.disabled = true; btn.textContent = '전송 중...';
    try { await sendResetEmail(emailValue); showToast('재설정 메일을 보냈습니다.', 'success'); btn.textContent = '메일 전송 완료'; }
    catch (err) { showToast(err.message || '메일 전송에 실패했습니다.', 'error'); btn.disabled = false; btn.textContent = '재설정 메일 보내기'; }
  });
