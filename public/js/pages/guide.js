export function renderGuide(container) {
  const steps = [
    ...
    ['🔒', '기본은 비공개, 공개는 직접 선택', '사건과 판결문은 본인에게만 보이는 것이 기본입니다. 공개로 전환하면 공개 판결기록이 되고 민심소의 투표·논쟁 대상이 됩니다.']
  ];

  const participationSteps = [
    ['1', '공개 판결 읽기', '민심소에서 다른 이용자가 공개한 생활사건을 판결문을 가린 채 먼저 읽습니다.'],
    ['2', '선택형 투표 참여', '원고가 더 억울한지, 피고 말도 이해되는지, 둘 다 잘못했는지처럼 정해진 반응 중 하나를 선택하면 AI 판결이 열립니다.'],
    ['3', '토론에서 의견 나누기', '투표한 그 자리에서 같은 사건을 두고 다른 배심원들과 댓글로 논쟁합니다. 개인정보 추측이나 인신공격은 허용되지 않습니다.'],
    ['4', '명예의 전당 구경', '쌓인 판정과 논쟁으로 줄 세운 화제·논란·억울지수 랭킹을 명예의 전당에서 구경합니다.']
  ];
  ...
  const faqs = [
    ...
    ['판결 결과를 공개하면 어떻게 되나요?', '안전하게 정리된 사건과 AI 판결문이 공개 판결기록이 되어 민심소의 투표·논쟁 대상이 되고, 반응이 쌓이면 명예의 전당 랭킹에도 오릅니다. 공개 주소는 검색엔진에 노출될 수 있으며, 다시 비공개로 전환하거나 사건을 삭제할 수 있습니다.'],
    ...
    <div>
      ...
      <div class="container" style="padding-top:28px;padding-bottom:90px;">
        ...
        <section aria-labelledby="participation-guide-title" style="margin-bottom:36px;">
          ...
          <p style="font-size:12px;color:var(--cream-dim);margin-bottom:16px;">별도 글 작성 없이 민심소에서 공개 판결을 맞히고, 투표한 자리에서 바로 논쟁하며, 그 결과는 명예의 전당 랭킹으로 쌓입니다.</p>
