...
const FEED_WRITE_TYPES = [
  ...
  { type:'영상 리액션', badge:'🎬', desc:'유튜브/쇼츠 링크 기능 확장에 맞춘 영상 반응형 글입니다. 지금은 링크를 본문에 넣어 사용할 수 있습니다.', titlePlaceholder:'예: 이 영상 한 줄 요약하면?', contentPlaceholder:'영상 링크와 웃긴 장면 설명을 적어주세요. 영상 기능 추가 전까지는 본문 링크로 공유됩니다.', questionPlaceholder:'예: 이 영상의 제일 웃긴 포인트는?', options:['웃김','킹받음','공감됨','댓글로 요약'] },
...
const QUICK_IDEAS = [
  ...
  { type:'소소토론', title:'카톡 답장 3시간 뒤면 서운하다?', content:'읽씹은 아닌데 답장이 늦을 때, 이 정도면 서운한지 궁금합니다.', question:'3시간 뒤 답장, 서운한가요?', options:['서운하다','괜찮다','상황마다 다르다','읽씹보다 낫다'] },
  { type:'퀴즈', title:'이 상황에서 제일 센스 있는 답은?', content:'정답보다 센스가 중요한 문제입니다. 가장 자연스러운 답을 골라주세요.', question:'제일 센스 있는 답은?', options:['1번','2번','3번','댓글이 정답'] },
  { type:'AI놀이', title:'AI가 만든 최악의 변명 고르기', content:'AI에게 변명을 만들어 달라고 했더니 이상한 답변이 나왔습니다.', question:'가장 웃긴 버전은?', options:['너무 진지함','말이 안 됨','오히려 설득됨','댓글로 이어가기'] },
...
function renderFeedWrite(container) {
  ...
  container.innerHTML = `
    <main class="predict-app soso-feed-page">
      <div class="simple-header feed-write-header"><a href="#/feed" class="back-link">‹</a><div><span>SOSO WRITE</span><h1>소소피드 만들기</h1></div><b>재미 유형</b></div>
      <section class="write-layout">
        <form id="feed-write-form" class="write-card fun-write-card">
          <div class="fun-write-hero"><span>MAKE IT FUN</span><h2>어떤 놀이로 올릴까요?</h2><p>게임 메뉴는 피드 안으로 합쳤습니다. 유형을 고르고 랜덤 주제를 눌러 바로 시작해보세요.</p><div><button type="button" id="random-idea-btn">🎲 랜덤 주제</button><a href="#/mission">🎯 미션 보기</a></div></div>
        ...
        <aside class="write-preview"><b>미리보기</b><div class="feed-card preview-card"><div class="feed-card-top"><span id="preview-type-label">${escapeHtml(config.badge)} ${escapeHtml(config.type)}</span><b>미리보기</b></div><img id="preview-image" class="feed-image" alt="미리보기 이미지" hidden /><h3 id="preview-title">제목을 입력하면 여기에 표시됩니다</h3><p id="preview-content">본문 설명과 참여 질문 후보가 붙으면 하나의 소소피드 카드가 됩니다.</p><div class="feed-question"><b id="preview-question">${escapeHtml(config.questionPlaceholder.replace(/^예:\s*/, ''))}</b><div class="feed-option"><span id="preview-option-a">${escapeHtml(config.options[0])}</span><i style="--w:72%"></i></div><div class="feed-option"><span id="preview-option-b">${escapeHtml(config.options[1])}</span><i style="--w:48%"></i></div></div><div class="feed-top-comment"><b>인기 한 줄 예시</b><span>이건 제목만 봐도 상황이 그려짐.</span></div></div><div class="side-card caution"><b>재미 포인트</b><p>사진, 선택지, 댓글, 투표가 붙으면 하나의 작은 게임이 됩니다. 영상/이미지 링크 기능은 다음 확장 단계로 넣으면 좋습니다.</p></div></aside>
      </section>
    </main>`;
...
function bindWriteForm(container) {
  ...
  const form = container.querySelector('#feed-write-form'); const status = container.querySelector('#feed-write-status');
  const updatePreview = () => {
  ...
  const fillIdea = (idea) => {
    container.querySelectorAll('.write-tabs button').forEach(item => item.classList.toggle('active', item.dataset.type === idea.type));
    applyTypeConfig(idea.type); container.querySelector('#feed-title').value = idea.title; container.querySelector('#feed-content').value = idea.content; container.querySelector('#feed-question').value = idea.question; container.querySelector('#feed-tags').value = `${idea.type}, 미션, 참여`;
