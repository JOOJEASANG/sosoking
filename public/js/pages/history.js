...
function fmtDay(day) {
  return `DAY ${String(day || 1).padStart(2, '0')}`;
}
...
function ensureHistoryStyle() {
  ...
  style.textContent = `
    ...
    .history-hero{border-radius:28px;padding:24px 20px;margin-bottom:14px;background:linear-gradient(135deg,rgba(15,23,42,.98),rgba(67,56,202,.82));color:#fff;box-shadow:0 18px 42px rgba(15,23,42,.20)}
    .history-hero__eyebrow{font-size:11px;font-weight:1000;letter-spacing:.09em;color:rgba(255,255,255,.62)}
    .history-hero__title{font-size:28px;font-weight:1000;margin:6px 0 4px;color:#fff;line-height:1.15}
    .history-hero__sub{font-size:14px;line-height:1.6;color:rgba(255,255,255,.76);max-width:720px}
    .history-hero__chips{display:flex;gap:7px;flex-wrap:wrap;margin-top:14px}
    .history-chip{border-radius:999px;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.15);padding:6px 9px;font-size:12px;font-weight:900;color:#fff}
    .history-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
    .history-card{border:1px solid rgba(100,116,139,.16);border-radius:22px;background:var(--color-surface,#fff);padding:15px;text-align:left;box-shadow:0 10px 24px rgba(15,23,42,.055);cursor:pointer;font-family:inherit;color:inherit}
    .history-card:hover{transform:translateY(-1px);box-shadow:0 14px 30px rgba(15,23,42,.08)}
    .history-card__meta{display:flex;gap:7px;flex-wrap:wrap;margin-bottom:9px}
    .history-card__tag{border-radius:999px;background:rgba(99,102,241,.09);color:#4f46e5;font-size:11px;font-weight:1000;padding:5px 8px}
    .history-card__title{font-size:17px;font-weight:1000;color:var(--color-text-primary);margin-bottom:5px;line-height:1.28}
    .history-card__summary{font-size:13px;line-height:1.55;color:var(--color-text-secondary);margin-bottom:10px}
    .history-card__question{font-size:12px;line-height:1.45;color:var(--color-text-muted);border-top:1px dashed rgba(100,116,139,.22);padding-top:9px}
    .history-detail{border-radius:24px;background:var(--color-surface,#fff);border:1px solid rgba(100,116,139,.16);box-shadow:0 14px 34px rgba(15,23,42,.07);overflow:hidden;margin-bottom:14px}
    .history-detail__head{padding:18px;background:linear-gradient(135deg,rgba(255,255,255,.98),rgba(248,250,252,.94));border-bottom:1px solid rgba(100,116,139,.12)}
    .history-detail__back{border:0;border-radius:999px;background:rgba(99,102,241,.09);color:#4f46e5;font-weight:1000;padding:8px 10px;font-family:inherit;cursor:pointer;margin-bottom:12px}
    .history-detail__title{font-size:24px;font-weight:1000;color:var(--color-text-primary);line-height:1.2}
    .history-detail__summary{font-size:14px;line-height:1.65;color:var(--color-text-secondary);margin-top:8px}
    .history-detail__body{padding:16px;display:grid;gap:12px}
    ...
    .history-choice__title{font-size:14px;font-weight:1000;color:var(--color-text-primary);margin-bottom:6px}
    .history-choice__stance{font-size:12px;line-height:1.55;color:var(--color-text-secondary)}
    .history-result{display:none;border-radius:18px;background:linear-gradient(135deg,rgba(15,23,42,.96),rgba(51,65,85,.92));color:#fff;padding:14px;margin-top:10px}
...
function renderHero(count) {
  return `<div class="history-hero">
    <div class="history-hero__eyebrow">HISTORY POLITICS SIMULATION</div>
    <div class="history-hero__title">📚 역사정치 자료실</div>
    <div class="history-hero__sub">실제 한국 현대사의 사건·제도·쟁점을 모티브로 삼고, 인물과 정당은 모두 가상화한 선택형 정치 시뮬레이션입니다.</div>
    <div class="history-hero__chips">
      <span class="history-chip">실제 사건 모티브</span>
      <span class="history-chip">가상 인물·정당</span>
      <span class="history-chip">선택 결과 비교</span>
      <span class="history-chip">${count || 0}개 사건</span>
    </div>
  </div>`;
}
...
function renderArchive(events) {
  return `<div class="history-grid">
    ${events.map(e => `<button type="button" class="history-card" data-day="${e.day}">
      <div class="history-card__meta">
        <span class="history-card__tag">${fmtDay(e.day)}</span>
        <span class="history-card__tag">${escHtml(e.era)}</span>
        <span class="history-card__tag">${escHtml(e.motifYear)}년 모티브</span>
      </div>
      <div class="history-card__title">${escHtml(e.title)}</div>
      <div class="history-card__summary">${escHtml(e.summary)}</div>
      <div class="history-card__question">질문 · ${escHtml(e.question)}</div>
    </button>`).join('')}
  </div>`;
}
...
function renderDetail(event) {
  ...
  return `<div class="history-detail">
    <div class="history-detail__head">
      ...
      <div class="history-card__meta">
        <span class="history-card__tag">${fmtDay(event.day)}</span>
        <span class="history-card__tag">${escHtml(event.era)}</span>
        <span class="history-card__tag">${escHtml(event.motifYear)}년 모티브</span>
      </div>
      <div class="history-detail__title">${escHtml(event.title)}</div>
      <div class="history-detail__summary">${escHtml(event.summary)}</div>
    </div>
    <div class="history-detail__body">
      <div class="history-box">
        <div class="history-box__title">📌 실제 역사 모티브</div>
      ...
      <div class="history-box">
        <div class="history-box__title">🏛️ 가상 정당 선택</div>
        <div class="history-choice-grid">
          ${(event.choices || []).map(choice => {
            const meta = event.partyMeta?.[choice.partyId] || {};
            return `<button type="button" class="history-choice" data-choice="${escHtml(choice.id)}" style="--party-color:${escHtml(meta.color || '#6366f1')}">
              <div class="history-choice__title">${escHtml(choice.emoji)} ${escHtml(choice.title)}</div>
              <div class="history-choice__stance">${escHtml(choice.stance)}</div>
            </button>`;
          }).join('')}
        </div>
      ...
      <div class="history-box">
        ...
        <div class="history-box__text">${escHtml(event.notice)}</div>
      </div>
      <div class="history-box">
        <div class="history-box__title">🔍 더 찾아볼 검색어</div>
        <div class="history-score">${(event.sourceGuide || []).map(s => `<span>${escHtml(s)}</span>`).join('')}</div>
      </div>
      <div class="history-actions">
        <button class="btn btn--primary" id="history-go-battle">오늘 역사정치게임 하기</button>
        <button class="btn btn--ghost" id="history-go-republic">공화국 현황</button>
      </div>
    </div>
  </div>`;
}
...
async function loadArchive(el) {
  const { events = [], count = 0 } = await call('getHistoryArchive', { limit: 80 });
  ...
  el.querySelectorAll('[data-day]').forEach(btn => {
    btn.addEventListener('click', () => navigate(`/history?day=${btn.dataset.day}`));
  });
}

async function loadDetail(el, day) {
  const { event } = await call('getHistoryEvent', { day });
  ...
  el.querySelectorAll('[data-choice]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      result.innerHTML = `<div class="history-result__title">${escHtml(choice.emoji)} 당신의 선택 · ${escHtml(choice.title)}</div>
        <div class="history-result__text">${escHtml(choice.result)}</div>
        <div class="history-score">${renderScores(choice.scores)}</div>`;
    });
  });
}
