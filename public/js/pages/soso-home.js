...
function card(item) {
  return `<a class="soso-recent-card" href="#/feed/${encodeURIComponent(item.id)}"><span>${e(item.badge || '✨')} ${e(item.type || '소소피드')}</span><h3>${e(item.title)}</h3><p>${e(item.summary || item.content || '')}</p><small>댓글 ${Number(item.stats?.comments || 0).toLocaleString()} · 좋아요 ${Number(item.stats?.likes || 0).toLocaleString()}</small></a>`;
}
