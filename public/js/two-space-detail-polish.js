...
// 토론소/드립소 상세 페이지의 참여 흐름을 한 번 더 정리합니다.
// - 토론글: 투표 → 내 의견 남기기 흐름으로 보정
// - 드립글: 중복 드립 입력 모듈을 숨기고 댓글형 드립 등록만 유지
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .multi-detail-root--drip-hidden{display:none!important}
...
function hideLegacyDripModule() {
  ...
  if (root && !root.querySelector('[data-multi-module="vote"]:not([hidden])')) {
    root.classList.add('multi-detail-root--drip-hidden');
  }
}
...
function polishDripOpinion() {
  ...
  const hint = section.querySelector('.multi-module-hint');
  if (hint) hint.textContent = '별도 참여창 없이 댓글처럼 한 줄 드립을 바로 등록합니다.';
  const input = section.querySelector('#comment-input');
  if (input && !input.dataset.polishedPlaceholder) {
