  ...
  return `
    <div class="soso-feed-search" role="search">
      <div class="soso-feed-search__input-wrap">
        ...
        <input id="feed-search-input" class="soso-feed-search__input" placeholder="정당·공약·시민발언 검색" value="${escHtml(search)}" autocomplete="off" inputmode="search">
...
export function renderFeedSortSelect({ sort = 'latest' } = {}) {
  return `
    <label class="soso-feed-sort soso-feed-sort--summary" aria-label="시민광장 정렬">
      <span>정렬</span>
...
export function renderFeedSummary({ total = 0, page = 1, totalPages = 1, search = '', type = '', sort = 'latest' } = {}) {
  const label = search ? `검색 ${escHtml(search)}` : (type ? TYPE_LABELS[type] || '필터' : '전체여론');
...
export function renderFeedEmptyState({ search = '' } = {}) {
  return `
    <div class="empty-state">
      <div class="empty-state__icon">${search ? '🔍' : '🏛️'}</div>
      <div class="empty-state__title">${search ? `"${escHtml(search)}" 검색 결과가 없어요` : '아직 시민발언이 없어요'}</div>
      <div class="empty-state__desc">${search ? '다른 검색어로 다시 찾아보세요.' : '첫 번째 시민발언을 남기고 공화국 여론을 시작해보세요.'}</div>
      ${search
        ? `<button class="btn btn--ghost" style="margin-top:16px" onclick="navigate('/feed')">전체여론 보기</button>`
        : `<button class="btn btn--primary" style="margin-top:16px" onclick="navigate('/write')">+ 시민발언 작성</button>`}
    </div>`;
}
