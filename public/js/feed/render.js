  ...
  return `
    <div class="feed-search-bar feed-search-bar--compact">
      <div class="feed-search-input-wrap feed-search-input-wrap--compact">
        ...
        <input id="feed-search-input" class="feed-search-input" placeholder="검색" value="${escHtml(search)}" autocomplete="off">
...
export function renderFeedEmptyState({ search = '' } = {}) {
  return `
    <div class="empty-state">
      ...
      <div class="empty-state__desc">${search ? '다른 검색어는 어때요?' : '첫 번째 피드 글을 올려볼까요?'}</div>
      ${!search ? `<button class="btn btn--primary" style="margin-top:16px" onclick="navigate('/write?type=multi')">+ 피드 만들기</button>` : ''}
    </div>`;
}
