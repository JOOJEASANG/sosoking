...
export const FALLBACK_FEED_ITEMS = [
  { id:'sample-office-chat', type:'생각 갈림', badge:'💬', title:'퇴근 후 단톡방 업무 지시, 어디까지 이해 가능?', summary:'회사 단톡방이 퇴근 후에도 계속 울릴 때 사람들은 어디까지 참을 수 있을까요?', question:'이 상황에서 제일 가까운 생각은?', options:['급한 일이면 가능','다음날 해도 된다','계속되면 선 넘음','읽씹한다'], votes:{'급한 일이면 가능':24,'다음날 해도 된다':31,'계속되면 선 넘음':41,'읽씹한다':7}, stats:{ views:12840, likes:932, comments:214 }, topComment:'한두 번은 괜찮은데 반복되면 퇴근이 퇴근이 아님.', tags:['직장','단톡방','퇴근'], source:'sample' },
  { id:'sample-title-school', type:'사진 제목학원', badge:'📸', title:'이 사진 제목 뭐가 제일 웃김?', summary:'평범한 사진도 제목 하나로 웃긴 글이 됩니다. AI 후보와 유저 드립이 붙는 참여형 글입니다.', question:'이 장면에 제일 어울리는 제목은?', options:['월요일 아침 내 표정','회의 끝난 줄 알았는데','퇴근 1분 전 부장님','오늘도 참았다'], votes:{'월요일 아침 내 표정':36,'회의 끝난 줄 알았는데':22,'퇴근 1분 전 부장님':58,'오늘도 참았다':19}, stats:{ views:23620, likes:1840, comments:356 }, topComment:'퇴근 1분 전 부장님은 너무 현실이라 웃프다.', tags:['웃긴사진','제목학원','드립'], source:'sample' },
  { id:'sample-late-friend', type:'소소한 논쟁', badge:'🤔', title:'친구가 매번 늦는데 사과는 잘한다면?', summary:'사과는 하지만 계속 반복되는 지각. 사람들은 이해할까요, 거리 둘까요?', question:'당신이라면 어떻게 할까요?', options:['계속 봐준다','한 번 제대로 말한다','슬슬 거리 둔다','이미 손절이다'], votes:{'계속 봐준다':13,'한 번 제대로 말한다':46,'슬슬 거리 둔다':32,'이미 손절이다':9}, stats:{ views:9870, likes:715, comments:168 }, topComment:'사과보다 중요한 건 다음에 안 늦는 거라고 봄.', tags:['친구','인간관계','공감'], source:'sample' },
  { id:'sample-restaurant-kid', type:'생활 매너', badge:'🍽️', title:'식당에서 아이가 계속 뛰어다닐 때, 누가 말해야 할까?', summary:'부모가 먼저 제지해야 할지, 가게도 말해야 할지 은근히 의견이 갈리는 생활 이슈입니다.', question:'가장 맞는 생각은?', options:['부모가 먼저','가게도 말해야 함','상황 봐야 함','애라서 어쩔 수 없음'], votes:{'부모가 먼저':62,'가게도 말해야 함':24,'상황 봐야 함':12,'애라서 어쩔 수 없음':2}, stats:{ views:15420, likes:1104, comments:287 }, topComment:'애가 문제가 아니라 방치하는 어른이 문제인 경우가 많음.', tags:['식당','매너','육아'], source:'sample' }
];
