export function applySubmitPrivacyDefaults(container) {
  ...
  if (labelText) {
    labelText.innerHTML = `<b style="color:var(--gold);">재판 완료 후 공개 선택</b><br><span style="color:var(--cream-dim);">기본값은 비공개입니다. 공개하면 닉네임, 사건 내용, 수사기록, 원고·피고 주장과 판결이 공개 재판기록에 표시됩니다. 원본 첨부 이미지는 작성자에게만 표시됩니다.</span>`;
  }

  const disclaimer = container.querySelector('.disclaimer');
  if (disclaimer && !disclaimer.textContent.includes('기본 비공개')) {
    disclaimer.innerHTML += '<br>· 사건과 재판 기록은 <strong>기본 비공개</strong>이며, 결과 화면에서 공개로 전환할 수 있습니다.';
  }
}
