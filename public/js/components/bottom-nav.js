...
function iconPlus() {
  return svgIcon('<path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15M4.5 12h15"/>', '2.2');
}
...
function iconAccount() {
  return svgIcon('<path stroke-linecap="round" stroke-linejoin="round" d="M15.75 7a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.5 21a7.5 7.5 0 0 1 15 0"/>');
}
...
const NAV_ITEMS = [
  ...
  { id: 'feed', label: '피드', path: '/feed', icon: iconPlus(), isCenter: true },
  { id: 'stats', label: '통계', path: '/hall', icon: iconStats() },
        ...
        if (item.isCenter) {
          return `
            <div class="bottom-nav__write">
              <button class="bottom-nav__write-btn${isActive ? ' active' : ''}" data-nav-path="${item.path}" aria-label="${item.label}" aria-current="${isActive ? 'page' : 'false'}">
                ${item.icon}
                <span class="bottom-nav__center-label">${item.label}</span>
              </button>
            </div>`;
        }
        return `
