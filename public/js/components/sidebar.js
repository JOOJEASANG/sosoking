...
function iconBell(){return svgIcon('<path stroke-linecap="round" stroke-linejoin="round" d="M15 17H9m9-2v-4a6 6 0 1 0-12 0v4l-2 2h16l-2-2ZM10 20a2 2 0 0 0 4 0"/>');}
...
function isNavActive(navPath,currentPath){return currentPath===navPath||(navPath==='/sosoland'&&currentPath.startsWith('/game/'));}
...
export function renderSidebar(){
  const el=document.getElementById('site-sidebar'); if(!el)return;
  const user=appState.user; const isAdmin=appState.isAdmin; const path=window.location.hash.slice(1).split('?')[0]||'/'; const dark=isDark(); const unread=appState.unreadNotifications||0;
  const NAV_ITEMS=[
    ...
    ...(isAdmin?[{label:'관리자',path:'/admin',icon:iconAdmin()}]:[]),
  ];
  const navHTML=NAV_ITEMS.map(item=>{const active=isNavActive(item.path,path);return `<a href="#${item.path}" class="sidebar__nav-item${active?' active':''}" aria-current="${active?'page':'false'}" data-nav="${item.path}">${item.icon}<span>${item.label}</span></a>`;}).join('');
  const userSection=user?`<div class="sidebar__user-wrap"><div class="sidebar__user"><div class="sidebar__user-avatar" id="sb-avatar" title="${escHtml(user.displayName||'내 정보')}" role="button" tabindex="0" aria-label="내 정보">${escHtml((user.displayName||appState.nickname||'나')[0])}</div><span class="sidebar__user-name" id="sb-username">${escHtml(appState.nickname||user.displayName||user.email||'사용자')}</span><a href="#/account?tab=notifications" class="sidebar__icon-btn" title="알림" aria-label="알림">${iconBell()}${unread>0?`<span class="sidebar__badge">${unread>99?'99+':unread}</span>`:''}</a></div><button class="sidebar__logout-btn" id="sb-logout-btn" title="로그아웃" aria-label="로그아웃">${svgIcon('<path stroke-linecap="round" stroke-linejoin="round" d="M15 8V5a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h7a2 2 0 0 0 2-2v-3M10 12h10m0 0-3-3m3 3-3 3"/>')}<span>로그아웃</span></button></div>`:`<a href="#/login" class="sidebar__login-btn">로그인 / 가입</a>`;
  el.innerHTML=`<div class="sidebar__logo"><a href="#/" class="sidebar__brand" aria-label="소소킹 홈" data-nav="/"><img src="/logo.svg" alt="" width="28" height="28"><span class="sidebar__brand-name">소소킹</span></a></div><nav class="sidebar__nav" aria-label="주 내비게이션">${navHTML}</nav><div class="sidebar__write"><button class="sidebar__write-btn" id="sb-write-btn" aria-label="피드 만들기">${iconWrite()}<span>피드 만들기</span></button></div><div class="sidebar__bottom">${userSection}<div class="sidebar__footer-utils"><button class="sidebar__util-btn" id="sb-theme-btn" aria-label="${dark?'라이트 모드로 전환':'다크 모드로 전환'}">${dark?iconSun():iconMoon()}<span>${dark?'라이트 모드':'다크 모드'}</span></button>${(appState.installPrompt||isIOS())&&!isStandalone()?`<button class="sidebar__util-btn" id="sb-pwa-btn" aria-label="앱 설치">${iconInstall()}<span>앱 설치</span></button>`:''}</div></div>`;
  el.querySelectorAll('[data-nav]').forEach(link=>{link.addEventListener('click',e=>{e.preventDefault();navigate(link.dataset.nav);});});
  document.getElementById('sb-write-btn')?.addEventListener('click',()=>{ if(!appState.user){navigate('/login?return=/write?type=multi');}else{navigate('/write');} });
  ...
  document.getElementById('sb-pwa-btn')?.addEventListener('click',async()=>{const prompt=appState.installPrompt;if(prompt){prompt.prompt();const{outcome}=await prompt.userChoice;if(outcome==='accepted'){appState.installPrompt=null;renderSidebar();}}else showIOSInstallGuide();});
}

window.addEventListener('hashchange',()=>{const el=document.getElementById('site-sidebar');if(!el)return;const path=window.location.hash.slice(1).split('?')[0]||'/';el.querySelectorAll('[data-nav]').forEach(link=>{const active=isNavActive(link.dataset.nav,path);link.classList.toggle('active',active);link.setAttribute('aria-current',active?'page':'false');});});
