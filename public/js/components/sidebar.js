...
function renderNavItem(item, currentPath) {
  ...
  return `<a href="#${item.path}" class="${cls}" aria-current="${active ? 'page' : 'false'}" data-nav="${item.path}">${item.icon}<span>${item.label}</span></a>`;
}
...
export function renderSidebar() {
  ...
  const MAIN_NAV = [
    ...
    { label: '⚔️ 오늘게임', path: '/battle', icon: '' },
    { label: '📚 역사자료', path: '/history', icon: '' },
    { label: '🏛️ 정당·대선', path: '/republic', icon: '' },
    { label: '👑 대통령 선거', path: '/election', icon: '' },
  ];
  const PERSONAL_NAV = user ? [{ label: '내정보', path: '/account', icon: iconAccount() }] : [];
  ...
  const userSection = user ? `
    <div class="sidebar__user-wrap">
      <div class="sidebar__user">
        ...
        <div class="sidebar__user-info"><div class="sidebar__user-name" id="sb-username">${escHtml(nickname)}</div>${isAdmin ? '<div class="sidebar__user-role">🔑 관리자</div>' : ''}</div>
      </div>
      <button class="sidebar__logout-btn" id="sb-logout-btn" aria-label="로그아웃"><span>로그아웃</span></button>
    </div>` : `<a href="#/login" class="sidebar__login-btn">로그인 / 가입</a>`;

  el.innerHTML = `
    <div class="sidebar__logo">
      <a href="#/" class="sidebar__brand" aria-label="소소킹 홈" data-nav="/"><img src="/logo.svg" alt="" width="28" height="28"><span class="sidebar__brand-name">소소킹</span></a>
    </div>
    <nav class="sidebar__nav" aria-label="주 내비게이션">
      ${MAIN_NAV.map(i => renderNavItem(i, path)).join('')}
      ${PERSONAL_NAV.length ? `<div class="sidebar__nav-divider"></div>${PERSONAL_NAV.map(i => renderNavItem(i, path)).join('')}` : ''}
      ${ADMIN_NAV.length ? `<div class="sidebar__nav-divider"></div><div class="sidebar__nav-section-label">관리</div>${ADMIN_NAV.map(i => renderNavItem(i, path)).join('')}` : ''}
    </nav>
    <div class="sidebar__write"><button class="sidebar__write-btn" id="sb-play-btn" aria-label="오늘게임"><span style="font-size:18px">⚔️</span><span>오늘게임 하기</span></button></div>
    <div class="sidebar__bottom">
      ${userSection}
      <div class="sidebar__footer-utils"><button class="sidebar__util-btn" id="sb-theme-btn" aria-label="${dark ? '라이트 모드로 전환' : '다크 모드로 전환'}">${dark ? iconSun() : iconMoon()}<span>${dark ? '라이트 모드' : '다크 모드'}</span></button></div>
    </div>`;

  el.querySelectorAll('[data-nav]').forEach(link => link.addEventListener('click', e => { e.preventDefault(); navigate(link.dataset.nav); }));
  document.getElementById('sb-play-btn')?.addEventListener('click', () => navigate('/battle'));
