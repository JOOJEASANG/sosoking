...
export function renderSidebar() {
  ...
  const MAIN_NAV = [
    ...
    { label: '게임',     path: '/games', icon: iconGame() },
  ...
  el.innerHTML = `
    ...
    <nav class="sidebar__nav" aria-label="주 내비게이션">
      ${mainHTML}
      ${PERSONAL_NAV.length > 0 ? `<div class="sidebar__nav-divider"></div>${personalHTML}` : ''}
      ${ADMIN_NAV.length > 0 ? `<div class="sidebar__nav-divider"></div><div class="sidebar__nav-section-label">사이트 관리</div>${adminHTML}` : ''}
    </nav>
    <div class="sidebar__write"><button class="sidebar__write-btn" id="sb-write-btn" aria-label="게임 열기">${iconWrite()}<span>게임 열기</span></button></div>
    <div class="sidebar__bottom">
      ...
      <div class="sidebar__footer-utils">
        ...
        ${showInstall ? `<button class="sidebar__util-btn" id="sb-pwa-btn" aria-label="앱 설치">${iconInstall()}<span>앱 설치</span></button>` : ''}
      </div>
    </div>`;
  ...
  document.getElementById('sb-pwa-btn')?.addEventListener('click', async () => {
    ...
    if (prompt) {
      try {
        ...
        if (outcome === 'accepted') { window.__pwaInstallPrompt = null; appState.installPrompt = null; renderSidebar(); }
        else if (isIOS()) showIOSInstallGuide(); else showAndroidInstallGuide();
      } catch { if (isIOS()) showIOSInstallGuide(); else showAndroidInstallGuide(); }
    } else if (isIOS()) showIOSInstallGuide(); else showAndroidInstallGuide();
  });
}
