...
function svgIcon(path, strokeWidth = '1.8') { return `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="${strokeWidth}" aria-hidden="true">${path}</svg>`; }
...
function isNavActive(navPath, currentPath){return currentPath === navPath || (navPath === '/materials' && currentPath.startsWith('/material/'));}
...
export function renderSidebar() {
  ...
  const mainNav = [
    ...
    { label: '📰 오늘자료', path: '/today', icon: '' },
    { label: '📚 자료실', path: '/materials', icon: '' },
    { label: '💬 토론', path: '/debates', icon: '' },
  ];
  ...
  el.innerHTML = `<div class="sidebar__logo"><a href="#/" class="sidebar__brand" aria-label="소소킹 홈" data-nav="/"><img src="/logo.svg" alt="" width="28" height="28"><span class="sidebar__brand-name">소소킹</span></a></div><nav class="sidebar__nav" aria-label="주 내비게이션">${mainNav.map(i => renderNavItem(i, path)).join('')}${personalNav.length ? `<div class="sidebar__nav-divider"></div>${personalNav.map(i => renderNavItem(i, path)).join('')}` : ''}${adminNav.length ? `<div class="sidebar__nav-divider"></div><div class="sidebar__nav-section-label">관리</div>${adminNav.map(i => renderNavItem(i, path)).join('')}` : ''}</nav><div class="sidebar__write"><button class="sidebar__write-btn" id="sb-main-btn" aria-label="오늘자료"><span style="font-size:18px">📰</span><span>오늘자료 보기</span></button></div><div class="sidebar__bottom">${userSection}<div class="sidebar__footer-utils"><button class="sidebar__util-btn" id="sb-theme-btn" aria-label="${dark ? '라이트 모드로 전환' : '다크 모드로 전환'}">${dark ? iconSun() : iconMoon()}<span>${dark ? '라이트 모드' : '다크 모드'}</span></button></div></div>`;
  el.querySelectorAll('[data-nav]').forEach(link => link.addEventListener('click', e => { e.preventDefault(); navigate(link.dataset.nav); }));
  document.getElementById('sb-main-btn')?.addEventListener('click', () => navigate('/today'));
...
window.addEventListener('hashchange', () => {
  ...
  if (!el) return;
  const path = window.location.hash.slice(1).split('?')[0] || '/';
  el.querySelectorAll('[data-nav]').forEach(link => { const active = isNavActive(link.dataset.nav, path); link.classList.toggle('active', active); link.setAttribute('aria-current', active ? 'page' : 'false'); });
});
