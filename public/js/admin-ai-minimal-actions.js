...
async function renderMinimalAiPanel(force = false) {
  ...
  content.innerHTML = `
    <div id="ai-minimal-panel" style="display:flex;flex-direction:column;gap:18px;max-width:860px">
      <div>
        ...
        <div style="font-size:12px;color:var(--color-text-muted);line-height:1.6">AI 게시글은 자동 생성하지 않고, 아래에서 방 유형을 선택해 버튼을 누를 때 해당 유형 1개만 생성합니다.</div>
      </div>
      <div class="admin-stat-grid">
        ${renderStat('오늘 AI 사용량', `${used}/${limit || 0}`, `${pct}% 사용`)}
        ${renderStat('오늘 AI 모음', summary.todayAiPosts ?? '-', '수동 생성 포함')}
  ...
  document.getElementById('btn-ai-content-now')?.addEventListener('click', async event => {
    ...
    try {
      ...
      console.error(error);
      if (result) result.textContent = '❌ ' + (error.message || 'AI 모음 생성에 실패했어요');
      toast.error(error.message || 'AI 모음 생성에 실패했어요');
    } finally {
      btn.disabled = false;
...
function schedule() {
  clearTimeout(timer);
  timer = setTimeout(() => renderMinimalAiPanel(false), 180);
}

function forceSchedule() {
  clearTimeout(timer);
  timer = setTimeout(() => renderMinimalAiPanel(true), 120);
}

document.addEventListener('click', event => {
  if (event.target.closest?.('[data-admin-tab="ai"], [data-tab="ai"]')) forceSchedule();
}, true);
