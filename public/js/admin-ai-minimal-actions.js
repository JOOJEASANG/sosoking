...
const AI_POST_TYPES = [
  { key: 'collect', label: '모음방', desc: '유튜브·웃긴그림·링크 모음' },
...
async function renderMinimalAiPanel(force = false) {
  ...
  content.innerHTML = `<div id="ai-minimal-panel" style="display:flex;flex-direction:column;gap:18px;max-width:860px"><div><h2 class="admin-section-title">🤖 AI 관리</h2><div style="font-size:12px;color:var(--color-text-muted);line-height:1.6">AI 게시글은 자동 생성하지 않고, 아래에서 방 유형을 선택해 버튼을 누를 때 해당 유형 1개만 생성합니다.</div></div><div class="admin-stat-grid">${renderStat('오늘 AI 사용량', `${used}/${limit || 0}`, `${pct}% 사용`)}${renderStat('오늘 AI 모음', summary.todayAiPosts ?? '-', '수동 생성 포함')}${renderStat('미처리 신고', summary.unresolvedReports ?? '-', `자동숨김 ${summary.autoHiddenPosts || 0}건`)}</div><div class="card"><div class="card__body"><div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:14px"><div><div style="font-size:14px;font-weight:900">AI 기본 설정</div><div style="font-size:11px;color:var(--color-text-muted);margin-top:3px">AI 게시글 생성은 서버 환경 키 <b>GEMINI_API_KEY</b>를 사용합니다. API 키는 Firestore에 저장하지 않습니다.</div></div><button class="btn btn--primary btn--sm" id="btn-ai-minimal-save">설정 저장</button></div><div style="height:10px;border-radius:999px;background:var(--color-surface-2);overflow:hidden;margin-bottom:14px"><div style="height:100%;width:${pct}%;background:${pct >= 90 ? 'var(--color-danger)' : 'var(--color-primary)'}"></div></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px;margin-bottom:14px">${renderToggle('ai-enabled','AI 기능 사용',settings.enabled,'전체 AI 기능의 기본 사용 여부입니다.')}${renderToggle('ai-auto-content','AI 게시글 생성 사용',settings.aiAutoContentEnabled,'관리자가 직접 누를 때만 글을 생성합니다. 예약 자동생성은 중지됩니다.')}${renderToggle('ai-admin-automation','관리자 자동화',settings.aiAdminAutomationEnabled,'읽은 알림 정리, 신고 요약 등 운영 보조 기능입니다.')}${renderToggle('ai-auto-hide','신고 많은 글 자동숨김',settings.autoHideReportedPosts,'처음에는 꺼두는 것을 권장합니다.')}</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px"><label class="form-group" style="margin:0"><span class="form-label">일일 AI 호출 한도</span><input id="ai-daily-limit" class="form-input" type="number" min="0" max="100" value="${settings.aiDailyLimit}"></label><label class="form-group" style="margin:0"><span class="form-label">자동숨김 신고 기준</span><input id="ai-report-threshold" class="form-input" type="number" min="2" max="20" value="${settings.reportHideThreshold}"></label><label class="form-group" style="margin:0"><span class="form-label">읽은 알림 보관일</span><input id="ai-retention-days" class="form-input" type="number" min="7" max="365" value="${settings.notificationRetentionDays}"></label></div></div></div><div class="card"><div class="card__body"><div style="font-size:14px;font-weight:900;margin-bottom:6px">AI 모음 수동 생성</div><div style="font-size:12px;color:var(--color-text-muted);line-height:1.6;margin-bottom:12px">현재 사이트 구조에 맞춰 <b>모음방 / 토론방 / 퀴즈방 / 드립방</b> 데이터로 저장합니다. 선택한 방 유형 1개만 생성됩니다.</div><div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:center"><select class="form-input" id="ai-content-preset" aria-label="AI 모음 유형 선택">${renderAiPostTypeOptions()}</select><button class="btn btn--primary btn--sm" id="btn-ai-content-now">선택 유형 1개 생성</button></div><div id="ai-content-result" style="font-size:12px;color:var(--color-text-muted);margin-top:10px;line-height:1.6"></div></div></div><div class="card"><div class="card__body"><div style="font-size:14px;font-weight:900;margin-bottom:12px">관리자 자동화</div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn btn--ghost btn--sm" id="btn-ai-run-automation">관리자 자동화 실행</button></div><div style="font-size:11px;color:var(--color-text-muted);margin-top:10px">미션 자동생성과 예약 AI 글 자동생성은 사용하지 않습니다.</div></div></div></div>`;
  ...
  document.getElementById('btn-ai-content-now')?.addEventListener('click', async event => {
    ...
    const preset = document.getElementById('ai-content-preset')?.value || 'collect';
    ...
    if (!confirm(`${selected.label} AI 모음 1개를 생성할까요?`)) return;
    btn.disabled = true; btn.textContent = '생성 중...';
    if (result) result.textContent = `${selected.label} 모음을 생성 중입니다...`;
    try {
      const res = await generateAiContentNow({ preset, force: true });
      const data = res.data || {};
      const detailUrl = data.docId ? `#/detail/${encodeURIComponent(data.docId)}` : '';
      if (result) result.innerHTML = `✅ ${esc(data.typeLabel || selected.label)} 생성 완료: <b>${esc(data.title || '')}</b>${detailUrl ? ` · <a href="${detailUrl}" style="color:var(--color-primary);font-weight:900">글 보기</a>` : ''}`;
      toast.success(`${selected.label} AI 모음을 생성했어요`);
    } catch (error) { console.error(error); if (result) result.textContent = '❌ ' + (error.message || 'AI 모음 생성에 실패했어요'); toast.error(error.message || 'AI 모음 생성에 실패했어요'); }
    finally { btn.disabled = false; btn.textContent = '선택 유형 1개 생성'; }
  });
