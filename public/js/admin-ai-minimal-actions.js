...
async function renderMinimalAiPanel(force = false) {
  ...
  document.getElementById('btn-ai-content-now')?.addEventListener('click', async event => {
    ...
    } catch (error) { console.error(error); if (result) result.textContent = '❌ ' + (error.message || 'AI 게시글 생성에 실패했어요'); toast.error(error.message || 'AI 게시글 생성에 실패했어요'); }
