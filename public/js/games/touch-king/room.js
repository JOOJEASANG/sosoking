...
export async function createTouchKingRoom(options={}){
  ...
  const roundLimit=clampNumber(options.roundLimit,3,50,30);
  const roundSeconds=clampNumber(options.roundSeconds,8,30,12);
  const data={game:'touch-king',title:'소소터치왕',status:'waiting',phase:'lobby',code:makeRoomCode(),hostId:auth.currentUser.uid,hostName:gamePlayerName('방장'),maxPlayers,round:0,roundLimit,roundSeconds,boardSize:BOARD_SIZE,log:'인원수와 게임횟수를 정했습니다. 12개 그림 중 같은 그림을 가장 빨리 찾는 사람이 소소터치왕입니다.',createdAt:serverTimestamp(),updatedAt:serverTimestamp()};
...
export async function joinTouchKingRoom(r){
  await ensureGameGuestAuth();
  if(!auth.currentUser)throw new Error('게임 접속 정보를 확인하지 못했어요.');
  if(!r||r.game!=='touch-king')throw new Error('소소터치왕 방이 아닙니다.');
...
export async function renderTouchKingRoom(id){
  ...
  if(!snap?.exists()){el.innerHTML='<section class="symbol-spy"><div class="symbol-room-empty"><div>😢</div><h1>방을 찾을 수 없어요</h1><button class="symbol-spy__start" onclick="location.hash=\'/game/touch-king\'">소소터치왕으로</button></div></section>';return;}
  const first={id:snap.id,...snap.data()};
  if(first.game!=='touch-king'){
    el.innerHTML=`<section class="symbol-spy"><div class="symbol-room-empty"><div>⚠️</div><h1>소소터치왕 방이 아닙니다</h1><p>${esc(first.game||'알 수 없음')}</p><button class="symbol-spy__start" onclick="location.hash='/game/touch-king'">소소터치왕으로</button></div></section>`;
    return;
  }
...
function top(){return `<header class="symbol-spy__topbar"><button class="symbol-spy__ghost" type="button" data-back>← 소소터치왕</button><div class="symbol-spy__brand"><span>👑</span><b>${esc(room?.title||'소소터치왕')}</b><small>방 코드 ${esc(room?.code||'')}</small></div><div class="symbol-spy__score"><span>${players.length}/${Number(room?.maxPlayers||6)}명</span><span>${Number(room?.round||0)}/${Number(room?.roundLimit||30)}회</span></div></header>`;}
...
function lobbyHTML(){const joined=!!me(),host=isRoomHost(room),url=buildGameInviteUrl('touch-king',room.id);return `<section class="symbol-spy symbol-spy--room touch-king-game"><div class="touch-king-title">👑 소소터치왕</div>${top()}<div class="symbol-room"><article class="symbol-room__hero"><div class="symbol-vote__badge">12개 그림 빠른 터치 대결</div><h1>같은 그림을 가장 빨리 찾아라</h1><p>${esc(room.log||'초대 링크를 공유하고 참가자를 모아주세요.')}</p><div class="symbol-spy-explain"><span>1. 중앙판 12개와 내 판 12개를 봅니다.</span><span>2. 두 판에 동시에 있는 그림은 딱 1개입니다.</span><span>3. ${Number(room.roundLimit||30)}회 후 점수가 가장 높은 사람이 소소터치왕입니다.</span></div><div class="symbol-room__invite"><input class="form-input" readonly value="${esc(url)}"><button class="symbol-spy__start" type="button" data-copy>초대 링크 복사</button></div><div class="symbol-room__actions">${joined?'<button class="symbol-spy__ghost" type="button" data-ready>준비 상태 변경</button>':'<button class="symbol-spy__start" type="button" data-join>방 참가하기</button>'}${host?'<button class="symbol-spy__start" type="button" data-start>게임 시작</button>':''}</div></article>${panel(`설정: ${Number(room.maxPlayers||6)}명 · ${Number(room.roundLimit||30)}회 · 그림 ${BOARD_SIZE}개 · 게임당 ${Number(room.roundSeconds||12)}초`)}</div></section>`;}
function tile(s,dis=false){return `<button class="symbol-tile" type="button" data-symbol="${esc(s)}" ${dis?'disabled':''}><span>${s}</span></button>`;}
function playHTML(){const r=room.roundData||{},p=me(),b=p?(r.boards?.[p.uid]||[]):[],sel=!!p?.selectedSymbol,host=isRoomHost(room);const hint=sel?`내 선택: ${p.selectedSymbol} · ${p.selectedCorrect?'정답':'오답'}${p.responseMs?` · ${(Number(p.responseMs)/1000).toFixed(1)}초`:''}`:'12개 중 중앙판과 내 판에 동시에 있는 그림을 최대한 빨리 누르세요.';return `<section class="symbol-spy symbol-spy--room symbol-spy--room-play touch-king-game"><div class="touch-king-title">👑 소소터치왕</div>${top()}<div class="symbol-room-round"><div class="symbol-spy__playhead"><div><b>GAME ${Number(room.round||1)}/${Number(room.roundLimit||30)}</b><span>${hint}</span></div><div class="symbol-spy__timer" data-room-timer>${left()}</div></div><div class="symbol-spy__arena"><article class="symbol-board symbol-board--center"><div class="symbol-board__title">중앙판 · 12개</div><div class="symbol-board__grid touch-king-grid">${(r.center||[]).map(s=>tile(s,true)).join('')}</div></article><div class="symbol-spy__versus"><span>같은 그림 1개</span><b>12</b><small>빠를수록 고득점</small></div><article class="symbol-board symbol-board--player"><div class="symbol-board__title">내 판 · 12개</div><div class="symbol-board__grid touch-king-grid">${b.length?b.map(s=>tile(s,sel||left()<=0)).join(''):'<div class="symbol-room__none">방 참가 후 플레이할 수 있어요.</div>'}</div></article></div><div class="symbol-room-round__actions">${!p?'<button class="symbol-spy__start" type="button" data-join>방 참가하기</button>':''}${host?'<button class="symbol-spy__start" type="button" data-result>게임 결과</button><button class="symbol-spy__ghost" type="button" data-start>이번 게임 재시작</button>':''}</div>${panel('정답은 +100점, 빠른 보너스는 남은 시간 × 5점입니다. 오답은 -10점입니다.')}</div></section>`;}
function resultHTML(){const host=isRoomHost(room),final=Number(room.round||1)>=Number(room.roundLimit||30),r=room.roundData||{},sorted=rank();return `<section class="symbol-spy symbol-spy--room symbol-spy--room-result touch-king-game"><div class="touch-king-title">👑 소소터치왕</div>${top()}<div class="symbol-room-result"><article class="symbol-room__hero"><div class="symbol-result__badge">GAME ${Number(room.round||1)} 결과</div><h1>${final?'최종 소소터치왕':'게임 결과'}</h1><p>이번 정답은 <b>${r.common}</b> 입니다. ${final&&sorted[0]?`최종 우승자는 <b>${esc(sorted[0].name)}</b> 입니다.`:'다음 게임에서 역전할 수 있습니다.'}</p><div class="symbol-room-rank">${sorted.map((p,i)=>`<div><b>${i+1}. ${esc(p.name||'게스트')}</b><span>${Number(p.score||0)}점 · 정답 ${Number(p.correctCount||0)}개 · 평균 ${Number(p.correctCount||0)?((Number(p.totalMs||0)/Number(p.correctCount||1))/1000).toFixed(1):'-'}초</span></div>`).join('')}</div><div class="symbol-room__actions">${host?`<button class="symbol-spy__start" type="button" data-start>${final?'새 게임 시작':'다음 게임'}</button><button class="symbol-spy__ghost" type="button" data-lobby>대기방으로</button>`:''}</div></article>${panel(final?'최종 점수가 가장 높은 사람이 소소터치왕입니다.':'다음 게임을 시작하세요.')}</div></section>`;}
function bind(){
  ...
  document.querySelector('[data-result]')?.addEventListener('click',()=>phase('result','게임 결과가 공개되었습니다.'));
...
async function start(){
  ...
  const cur=Number(room.round||0),lim=Number(room.roundLimit||30),next=room.phase==='result'&&cur>=lim?1:cur+1,data=buildRound(next);
  ...
  await updateDoc(doc(db,'game_rooms',room.id),{status:'playing',phase:'playing',round:next,roundData:data,log:`GAME ${next} 진행 중. 12개 중 같은 그림을 가장 빨리 찾으세요!`,updatedAt:serverTimestamp()});}catch(e){toast.error(e.message||'게임 시작 실패');}
}
