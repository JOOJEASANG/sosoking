...
function style(){['/css/symbol-spy-room-sync.css','/css/symbol-spy-theme-polish.css'].forEach(href=>{if(document.querySelector(`link[href="${href}"]`))return;const l=document.createElement('link');l.rel='stylesheet';l.href=href;document.head.appendChild(l);});}
...
function buildRound(roundNo){
  ...
  return {round:roundNo,common,center,boards,startedAtMs:start,endsAtMs:start+Number(room?.roundSeconds||12)*1000};
}
...
export async function createTouchKingRoom(options={}){
  ...
  const data={game:'symbol-spy',gameMode:'touch-king',title:'터치왕게임',status:'waiting',phase:'lobby',code:makeRoomCode(),hostId:auth.currentUser.uid,hostName:gamePlayerName('방장'),maxPlayers,round:0,roundLimit,roundSeconds,log:'인원수와 판수를 정했습니다. 초대 링크를 공유하고 터치왕을 가려보세요.',createdAt:serverTimestamp(),updatedAt:serverTimestamp()};
...
export async function joinTouchKingRoom(r){
  await ensureGameGuestAuth();
  if(!auth.currentUser)throw new Error('게임 접속 정보를 확인하지 못했어요.');
  if(!r||r.game!=='symbol-spy')throw new Error('터치왕게임 방이 아닙니다.');
...
export async function renderTouchKingRoom(id){
  ...
  if(!snap?.exists()){el.innerHTML='<section class="symbol-spy"><div class="symbol-room-empty"><div>😢</div><h1>방을 찾을 수 없어요</h1><button class="symbol-spy__start" onclick="location.hash=\'/game/symbol-spy\'">터치왕게임으로</button></div></section>';return;}
  const first={id:snap.id,...snap.data()};
  if(first.game!=='symbol-spy'){el.innerHTML=`<section class="symbol-spy"><div class="symbol-room-empty"><div>⚠️</div><h1>터치왕게임 방이 아닙니다</h1><p>${esc(first.game)}</p></div></section>`;return;}
...
function playerState(p){if(room?.phase==='result')return `${Number(p.score||0)}점 · ${Number(p.correctCount||0)}개`;if(p.selectedSymbol)return p.selectedCorrect?`정답 · ${Number(p.responseMs||0)/1000}초`:'오답';return p.ready?'준비 완료':'대기 중';}
...
async function start(){
  ...
  await updateDoc(doc(db,'game_rooms',room.id),{status:'playing',phase:'playing',round:next,roundData:data,log:`ROUND ${next} 진행 중. 같은 그림을 가장 빨리 찾으세요!`,updatedAt:serverTimestamp()});}catch(e){toast.error(e.message||'라운드 시작 실패');}
}
