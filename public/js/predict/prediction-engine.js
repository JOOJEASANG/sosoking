...
const BOARDS = [
  {
    id: 'hot-issue-survive', status: 'open', category: '핫이슈 생존', title: '오늘 1위 이슈, 내일도 TOP5에 남을까?', issue: '오늘 여러 채널에서 반복 등장한 급상승 이슈', summary: '오늘 크게 오른 이슈가 하루짜리인지, 내일까지 살아남을지 예측하는 판입니다.', question: '내일 오후 6시 기준, 오늘의 1위 이슈가 다시 TOP5 안에 들어올까?',
    options: [{ id: 'survive', label: '남는다', odds: 1.7 }, { id: 'fade', label: '사라진다', odds: 1.9 }], closeAt: '오늘 23:59', resultAt: '내일 18:00', resultRule: '자동 수집된 내일 핫이슈 점수 TOP5 기준', heat: 91, participants: 1284, aiComment: 'AI는 이슈의 기사량, 반복 키워드, 검색 상승률을 기준으로 생존 가능성을 계산합니다.'
  },
  {
    id: 'trend-category-winner', status: 'open', category: '카테고리 예측', title: '내일 가장 뜨거운 카테고리는?', issue: '연예·스포츠·IT·날씨·콘텐츠 흐름 비교', summary: '하루 동안 가장 많이 움직인 카테고리가 내일도 이어질지 보는 예측판입니다.', question: '내일 핫이슈 TOP5 중 가장 많이 등장할 카테고리는?',
    options: [{ id: 'entertainment', label: '연예/방송', odds: 2.1 }, { id: 'sports', label: '스포츠', odds: 2.4 }, { id: 'it', label: 'IT/게임', odds: 2.8 }, { id: 'weather', label: '날씨/생활', odds: 2.2 }], closeAt: '오늘 23:59', resultAt: '내일 18:00', resultRule: '자동 수집 이슈의 카테고리 분류 기준', heat: 76, participants: 842, aiComment: '카테고리 예측은 한 이슈보다 변수가 많아서 역전 재미가 큰 판입니다.'
  },
  {
    id: 'second-place-reverse', status: 'open', category: '역전 예측', title: '현재 2위 이슈가 내일 1위로 올라설까?', issue: '오늘 2위권에서 빠르게 올라오는 추격 이슈', summary: '지금은 2위지만 댓글량과 검색 상승률이 강한 이슈가 내일 1위를 차지할지 맞힙니다.', question: '내일 오후 6시 기준, 현재 2위 이슈가 1위로 역전할까?',
    options: [{ id: 'reverse', label: '역전한다', odds: 2.6 }, { id: 'no_reverse', label: '못 한다', odds: 1.55 }], closeAt: '오늘 23:59', resultAt: '내일 18:00', resultRule: '내일 핫이슈 점수 1위 여부 기준', heat: 83, participants: 653, aiComment: '역전판은 소소머니를 크게 불릴 수 있지만, 그만큼 빗나갈 확률도 높습니다.'
  }
];
...
export async function claimDailyBonusAsync() {
  try {
    ...
    if (data.wallet) writeJson(WALLET_KEY, normalizeWallet(data.wallet));
    return { ok: true, ...data };
  } catch { return { ok: false, ...claimDailyBonus() }; }
}
...
export async function placePredictionAsync({ boardId, optionId, amount, comment }) {
  ...
  try {
    ...
    const data = res.data || {};
    if (data.wallet) writeJson(WALLET_KEY, normalizeWallet(data.wallet));
    rememberPrediction({ boardId: data.boardId || boardId, optionId, optionLabel: option.label, odds: option.odds, amount, comment, source: 'server' });
...
export async function addCommentAsync(boardId, text, side = '내 의견') {
  try {
    ...
    return { ok: true, comment: addComment(boardId, text, side, true) };
  } catch (error) { return { ok: false, error, comment: addComment(boardId, text, side, true) }; }
}
...
export function getComments(boardId, all = false) {
  ...
  const merged = [...stored, ...server, ...seeded.filter(s => !stored.some(x => x.id === s.id) && !server.some(x => x.id === s.id))];
...
function writeAllComments(comments) { const mineAndStored = comments.filter(c => c.mine || String(c.id).startsWith('custom-') || String(c.id).includes('comment-')); writeJson(COMMENTS_KEY, mineAndStored); }
