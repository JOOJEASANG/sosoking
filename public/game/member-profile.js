...
const GAME_GUIDES = {
  ...
  naming: {
    ...
    emoji: '✍️',
    title: '작명톡 생존전',
    meta: '2명~무제한 · 한 명씩 진행 · 차례당 30초',
    goal: '방장이 정한 주제에 맞는 새 이름을 채팅처럼 한 명씩 이어가며 마지막까지 살아남는 작명게임입니다.',
    steps: ['내 차례에 주제에 맞는 이름을 24자 안으로 입력합니다.', '등록된 이름은 모두의 채팅 기록에 즉시 쌓이고 다음 사람에게 차례가 넘어갑니다.', '더 떠오르지 않으면 포기를 누르며, 마지막 생존자가 우승합니다.'],
    scoring: ['처음 나온 유효한 이름 하나마다 1개 기록으로 인정됩니다.', '이미 나온 이름을 반복하거나 30초가 지나면 즉시 탈락합니다.', '점수보다 생존이 우선이며 마지막 한 명이 작명왕입니다.'],
    specials: ['대기실 참가 인원에는 별도 상한을 두지 않습니다.', '이름·포기·시간초과 기록은 세션별 Firestore 데이터로 남고 재경기해도 삭제되지 않습니다.', '한 차례에는 지정된 참가자 한 명만 기록을 만들 수 있습니다.'],
    tips: ['흔한 이름부터 쓰면 뒤로 갈수록 선택지가 빠르게 줄어듭니다.', '띄어쓰기와 기호만 바꾼 이름도 같은 이름으로 처리됩니다.', '내 차례가 오기 전에 두세 개 후보를 미리 생각해 두세요.']
  }
};
