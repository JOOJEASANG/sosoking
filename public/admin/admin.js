    ...
    <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;background:radial-gradient(ellipse at 50% 0%, rgba(201,168,76,0.07) 0%, transparent 55%);">
      <div style="width:100%;max-width:380px;">
        <div style="text-align:center;margin-bottom:36px;">
          <div style="font-size:56px;line-height:1;filter:drop-shadow(0 0 24px rgba(201,168,76,0.35));">🔥</div>
          <div style="font-family:'Noto Serif KR',serif;font-size:22px;color:var(--gold);margin-top:14px;font-weight:700;letter-spacing:-0.01em;">소소킹 토론배틀</div>
        ...
        <div style="text-align:center;margin-top:20px;font-size:11px;color:rgba(245,240,232,0.2);">소소킹 토론배틀 · 내부 전용</div>
      </div>
    </div>`;
    ...
    <div>
      <div class="admin-header">
        <div style="display:flex;align-items:center;gap:12px;">
          ...
          <div>
            <div class="admin-brand-title">소소킹 토론배틀</div>
...
async function tabUsage(el) {
  ...
  el.innerHTML = `
    ...
    <div style="margin-top:28px;padding:20px;background:rgba(231,76,60,0.05);border:1.5px solid rgba(231,76,60,0.25);border-radius:12px;">
      <div style="font-size:14px;font-weight:700;color:var(--cream);margin-bottom:4px;">🗑️ 배틀 데이터 정리</div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px;">
        <div>
          ...
          <select id="cleanup-status" class="admin-input" style="width:100%;">
            <option value="completed">판정 완료만</option>
...
제1조 (목적)
소소킹 토론배틀(이하 "서비스")는 일상의 사소한 갈등을 주제로 친구 또는 AI와 토론하고 AI 심판에게 재밌는 판정을 받는 순수 오락·엔터테인먼트 서비스입니다. 본 약관은 서비스 이용 조건 및 절차를 규정합니다.

제2조 (서비스의 성격)
① 본 서비스는 순수 오락 목적으로 제공되며, AI가 생성한 판정문은 어떠한 법적 효력도 없습니다.
...
제7조 (면책)
① 서비스 운영자는 AI가 생성한 판정 결과의 정확성, 적절성에 대해 책임을 지지 않습니다.
...
단, AI 판정·반박 생성을 위해 이용자가 입력한 사건 내용이 Google LLC의 Gemini API로 전송됩니다.
...
AI 판정은 어떠한 법적 효력도 없습니다.
━━━━━━━━━━━━━━━━━━━━

1. AI 사용 현황

[AI 심판]
매 배틀마다 7가지 성향 중 랜덤 배정된 AI 심판가 양측 주장을 분석해 오락용 판정문을 생성합니다.
...
3. 법적 효력 없음
본 서비스에서 생성된 모든 판정문은 법원 또는 어떠한 공적 기관의 판결과도 무관하며, 법적 효력이 전혀 없습니다.
...
· 판정 결과를 타인과 공유 시 오락 목적 서비스임을 명시해 주세요.
...
async function tabTopics(el) {
  ...
  const renderPendingRow = d => {
    ...
    return `<tr data-id="${d.id}">
      <td style="font-size:12px;">
        ...
        <div style="font-size:11px;color:var(--cream-dim);margin-top:4px;">🔴 ${(t.plaintiffPosition||'').substring(0,30)}...</div>
        <div style="font-size:11px;color:var(--cream-dim);">🔵 ${(t.defendantPosition||'').substring(0,30)}...</div>
      </td>
  ...
  el.innerHTML = `
    ...
    <div style="margin-bottom:24px;">
      <button id="new-topic-toggle" style="display:flex;align-items:center;gap:8px;width:100%;padding:12px 16px;background:rgba(201,168,76,0.06);border:1px solid rgba(201,168,76,0.3);border-radius:10px;color:var(--gold);font-size:13px;font-weight:700;cursor:pointer;text-align:left;">
        <span id="new-topic-arrow">▶</span> ✏️ 공식 주제 직접 등록
      </button>
      <div id="new-topic-form" style="display:none;margin-top:8px;padding:18px;background:rgba(255,255,255,0.02);border:1px solid var(--border);border-radius:10px;">
        <div style="display:grid;gap:12px;">
          <div><div class="form-label">주제명 <span style="color:var(--red);">*</span></div>
            <input type="text" id="nt-title" class="form-input" maxlength="30" placeholder="예: 카톡 읽씹 무죄 주장"></div>
          ...
          <div><div class="form-label">🔴 A팀 입장 <span style="color:var(--red);">*</span></div>
            <textarea id="nt-plaintiff" class="form-textarea" style="min-height:60px;" maxlength="100" placeholder="A팀 측 주장을 입력하세요"></textarea></div>
          <div><div class="form-label">🔵 B팀 입장 <span style="color:var(--red);">*</span></div>
            <textarea id="nt-defendant" class="form-textarea" style="min-height:60px;" maxlength="100" placeholder="B팀 측 주장을 입력하세요"></textarea></div>
          ...
          <button id="nt-submit" class="btn btn-primary" style="margin-top:4px;">🔥 공식 주제로 등록</button>
        </div>
      </div>
    </div>
    ...
    ${pendingSnap.docs.length?`
      ...
      <div class="admin-section-box" style="margin-bottom:24px;"><div style="overflow-x:auto;">
        <table class="admin-table"><thead><tr><th>주제 내용</th><th style="width:130px;">카테고리 · 처리</th></tr></thead>
        <tbody>${pendingSnap.docs.map(renderPendingRow).join('')}</tbody></table>
      </div></div>
    `:''}

    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:12px;">
      <div class="admin-section-title" style="margin:0;">✅ 공개 주제 <span style="background:rgba(201,168,76,0.12);color:var(--gold);border-radius:20px;padding:1px 8px;font-size:11px;">${activeSnap.docs.length}</span></div>
    ...
    <div class="admin-section-box" style="margin-bottom:24px;"><div style="overflow-x:auto;">
      <table class="admin-table"><thead><tr><th>주제명</th><th>A팀 주장</th><th style="text-align:center;width:60px;">배틀수</th><th>관리</th></tr></thead>
    ...
    ${hiddenSnap.docs.length?`
      ...
      <div class="admin-section-box"><div style="overflow-x:auto;">
        <table class="admin-table"><thead><tr><th>주제명</th><th>관리</th></tr></thead>
  ...
  document.getElementById('nt-submit')?.addEventListener('click', async () => {
    ...
    try {
      ...
      toast('주제가 등록되었습니다!', 'success');
      loadTab('topics');
    } catch(e) { toast('등록 실패: '+e.message, 'error'); btn.disabled=false; btn.textContent='⚖️ 공식 주제로 등록'; }
  });
    ...
    modalEl.innerHTML = `
      <div style="max-width:560px;margin:0 auto;background:var(--navy-card);border:1px solid var(--border);border-radius:16px;padding:28px;position:relative;box-shadow:0 24px 64px rgba(0,0,0,0.6);">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:22px;">
          <div style="font-family:'Noto Serif KR',serif;font-size:16px;font-weight:700;color:var(--gold);">✏️ 주제 수정</div>
  ...
  window._editTopic = async id => {
    ...
    if (!snap.exists()) { toast('주제를 찾을 수 없습니다','error'); return; }
    const t = snap.data();
