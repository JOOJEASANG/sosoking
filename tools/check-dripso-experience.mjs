...
assert.ok(baseIndex >= 0 && overrideIndex > baseIndex, '오늘의 한줄 서버 구현은 기존 드립소 함수 뒤에서 addDripsoComment를 덮어써야 합니다.');
...
for (const required of [
  ...
  assert.ok(courtGuard.includes(required), `판결소 상단 사이트 전환이 누락되었습니다: ${required}`);
}
...
for (const required of [
  ...
  assert.ok(courtCss.includes(required), `판결소 상단 전환 스타일이 누락되었습니다: ${required}`);
}

assert.ok(courtIndex.includes('/css/dripso-entry.css?v=20260802-dripso-navigation-1'), '판결소가 최신 사이트 전환 CSS를 불러와야 합니다.');
assert.ok(courtIndex.includes('/js/dripso-entry-guard.js?v=20260802-dripso-navigation-1'), '판결소가 최신 사이트 전환 스크립트를 불러와야 합니다.');

console.log('Dripso experience validation passed: daily comments are one-line only, naming is branded as 미친작명소, and both sites expose a top service switcher.');
