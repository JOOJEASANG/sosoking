...
const pages = ['grid', 'vault', 'chosung', 'mind', 'naming'].map(folder => read(`public/game/${folder}/index.html`));
...
for (const page of pages) {
  ...
  assert.match(page, /game-master\.js\?v=20260817-naming-1/);
}
for (const required of ['게임마스터', '자동 ON', '잠시멈춤', 'REVEAL_SELECTORS', 'NEXT_SELECTORS', 'data-vault', 'data-grid-action', 'RESULT_DELAY']) {
...
console.log('Auto game-master validation passed for all five live games, with Grid/Vault fallback actions and no retired Alibi phase controls.');
