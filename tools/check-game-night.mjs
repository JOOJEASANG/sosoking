...
for (const folder of liveGames) {
  ...
  assert.match(page, /game-night\.css\?v=20260817-grid-2/);
  assert.match(page, /game-night\.js\?v=20260817-grid-2/);
}

for (const required of [
  "type: 'grid-rush'", "type: 'vault-run'", "type: 'chosung-bomb'", "type: 'mind-reader'", "type: 'alibi-market'",
  ...
  'score: 0', 'location.replace(targetUrl(selected))', 'game.type === latestRoom.type',
...
for (const removed of ['dna-boss', 'normalizeDna', 'sosoking-world', 'greed-stairs', 'unique-low', 'laps:', 'damage:', 'runState:', "collection(db, 'game_rooms', roomId, 'reactions')"]) {
...
console.log('Game night validation passed: five games support secure score resets, same-game replay recovery, and same-room switching without DNA carryover.');
