...
if (!errors.length) {
  ...
  const pagination = read('public/dripso/pagination.js');
  ...
  for (const required of [
    ...
    if (!pagination.includes(required)) errors.push(`public/dripso/pagination.js: missing ${required}`);
  }
  for (const required of [
...
console.log('Dripso moderation validation passed: ownership deletion, reporting, administrator actions, pagination, verified admin email, and serialized deployment are connected.');
