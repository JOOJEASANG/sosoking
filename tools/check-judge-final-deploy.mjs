...
const runtimeVersion = '20260810-judge-runtime-2';
const trialAssetVersion = '20260810-current-judges-1';
...
for (const name of expected) {
  ...
  if (!trialPageJudgeBlock.includes(`['${name}',`)) fail(`재판 진행 화면 판사 누락: ${name}`);
  if (!trialPage.includes(`'${name}': [`)) fail(`재판 진행 판사별 진행문구 누락: ${name}`);
}
...
const trialPageTypes = [...trialPageJudgeBlock.matchAll(/\['([^']+)'/g)].map(match => match[1]);
...
if (trialPageTypes.join('|') !== expected.join('|')) fail(`재판 진행 화면 판사 순서 불일치: ${trialPageTypes.join(', ')}`);
...
for (const name of legacy) {
  ...
  if (trialPageJudgeBlock.includes(name)) fail(`재판 진행 화면에 구형 판사 잔존: ${name}`);
}
...
if (!runtimeGuard.includes('const observeTarget = document.body')) fail('라우팅 후 교체되는 page-content 때문에 런타임 판사 가드가 끊길 수 있습니다.');
if (!runtimeGuard.includes('looksLikeLegacyDripProgress(root)')) fail('현재 드립형과 구형 7번째 드립형을 구분하는 호환 로직이 없습니다.');
for (const [legacyName, currentName] of staleCompatibility) {
  if (!runtimeGuard.includes(`['${legacyName}', '${currentName}']`)) {
    fail(`구형 캐시 판사 호환 매핑 누락: ${legacyName} -> ${currentName}`);
  }
}

if (!trialGame.includes(`./trial.js?v=${trialAssetVersion}`)) fail('재판 진행 본체 캐시 키가 신규 판사 버전으로 갱신되지 않았습니다.');
if (!index.includes(`/js/judge-runtime-guard.js?v=${runtimeVersion}`)) fail('index.html 런타임 판사 가드 캐시 키가 갱신되지 않았습니다.');
...
console.log('Final judge deployment validation passed: generation, trial source, five-stage personas, home/result/guide UI, stale-cache trial compatibility, board chips, release markers, and all three production hosts are covered.');
