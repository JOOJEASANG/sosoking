...
const version = '20260801-pc-daily-icon-1';
assert.ok(app.includes(`./components/nav.js?v=${version}`), 'app.js가 수정된 하단 메뉴 버전을 불러와야 합니다.');
assert.ok(index.includes(`/js/app.js?v=${version}`), 'index.html이 수정된 app.js 버전을 불러와야 합니다.');
assert.ok(serviceWorker.includes(`/js/app.js?v=${version}`), '서비스워커가 수정된 app.js 버전을 선행 캐시해야 합니다.');
assert.ok(serviceWorker.includes(`/js/components/nav.js?v=${version}`), '서비스워커가 수정된 하단 메뉴 버전을 선행 캐시해야 합니다.');
assert.ok(serviceWorker.includes(`sosoking-app-v${version}`), '서비스워커 캐시 이름도 함께 갱신되어야 합니다.');
