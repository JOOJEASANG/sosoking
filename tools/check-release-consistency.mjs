...
for (const route of ['grid', 'vault', 'chosung', 'mind', 'naming']) {
  ...
  assert.match(page, /\/game\/lobby-tools\.css\?v=20260822-lobby-tools-1/);
  assert.match(page, /\/game\/lobby-tools\.js\?v=20260822-lobby-tools-1/);
}

console.log(`Release consistency passed for ${release}: runtime is game-only and live verification follows the deployed commit.`);
